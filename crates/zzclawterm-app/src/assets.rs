...
pub struct ZzClawTermAssets;

impl AssetSource for ZzClawTermAssets {
    fn load(&self, path: &str) -> Result<Option<Cow<'static, [u8]>>> {
        ...
        // names like `icons/search.svg`. Keep ZzClawTerm assets authoritative, then
...
mod tests {
    ...
    use super::{EmbeddedAssets, ZzClawTermAssets};
    ...
    fn list_returns_only_entries_below_the_requested_prefix() {
        let all = ZzClawTermAssets.list("").expect("list all");
        let icons = ZzClawTermAssets.list("icons").expect("list icons");

    ...
    loop {
        ...
        match lock_file.try_lock() {
            ...
            Err(std::fs::TryLockError::Error(error)) => {
                return Err(error).context("acquire ZzClawTerm single-instance lock");
            }
        }
        ...
        if started.elapsed() >= CONNECT_DEADLINE {
            bail!(
                "another ZzClawTerm instance owns this data directory but did not accept activation"
            );
        }
...
fn start_owner(
    ...
    listener
        ...
        .context("configure ZzClawTerm activation listener")?;
    ...
    let listener_thread = thread::Builder::new()
        .name("zzclawterm-activation-listener".to_string())
        .spawn(move || run_listener(listener, auth_token, listener_activation_tx, listener_stop))
        .context("spawn ZzClawTerm activation listener")?;
...
fn forward_activation(
    ...
    let mut stream = TcpStream::connect_timeout(&endpoint.address, IO_TIMEOUT)
        .context("connect to primary ZzClawTerm instance")?;
    ...
    match ack.status {
        ...
        ActivationAckStatus::QueueFull => bail!("primary ZzClawTerm activation queue is full"),
        ActivationAckStatus::ShuttingDown => bail!("primary ZzClawTerm instance is shutting down"),
        ActivationAckStatus::InvalidRequest => {
            bail!("primary ZzClawTerm instance rejected the activation request")
        }
    }
}
...
fn write_endpoint(
    ...
    std::fs::rename(&temporary_path, endpoint_path).with_context(|| {
        format!(
            "publish ZzClawTerm activation endpoint '{}'",
...
fn read_endpoint(endpoint_path: &Path) -> anyhow::Result<Option<ActivationEndpoint>> {
    let content = match std::fs::read_to_string(endpoint_path) {
        ...
        Err(error) => return Err(error).context("read ZzClawTerm activation endpoint"),
    };
    let mut lines = content.lines();
    if lines.next() != Some("ZZCTI1") {
        return Ok(None);
    }

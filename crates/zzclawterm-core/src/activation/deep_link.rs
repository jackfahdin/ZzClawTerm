...
fn parse_zzclawterm_url(url: &Url) -> Result<ActivationAction, ActivationParseError> {
    reject_common_suffix(url)?;
    ...
    match url.path() {
        "/ssh" => parse_zzclawterm_ssh_query(url.query()),
        "/telnet" => parse_zzclawterm_telnet_query(url.query()),
        _ => Err(ActivationParseError::UnsupportedAction),
    }
}

fn parse_zzclawterm_ssh_query(
    query: Option<&str>,
...
fn parse_zzclawterm_telnet_query(
    query: Option<&str>,
) -> Result<ActivationAction, ActivationParseError> {
    let mut host = None;
    let mut port = None;
...
mod tests {
    ...
    #[test]
    fn parses_strict_ssh_telnet_and_zzclawterm_links() {
        assert_eq!(
        ...
        assert_eq!(
            parse_deep_link("zzclawterm://connect/ssh?host=example.com&port=22&username=deploy"),
            Ok(ActivationAction::Connect(ExternalConnectionRequest::Ssh {
                host: "example.com".to_string(),
                port: 22,
        ...
        assert_eq!(
            parse_deep_link("zzclawterm://connect/telnet?host=example.com"),
            Ok(ActivationAction::Connect(
                ExternalConnectionRequest::Telnet {
                    host: "example.com".to_string(),
                    port: 23,
    ...
    fn rejects_credentials_suffixes_and_noncanonical_schemes() {
        for link in [
            ...
            "SSH://example.com",
            "https://example.com",
            "zzclawterm://user@connect/ssh?host=example.com",
        ] {
    ...
    fn rejects_unknown_duplicate_and_unsafe_query_values() {
        assert_eq!(
            parse_deep_link("zzclawterm://connect/ssh?host=a&host=b"),
            Err(ActivationParseError::DuplicateParameter)
        );
        for link in [
            "zzclawterm://connect/ssh?host=a&password=secret",
            "zzclawterm://connect/ssh?host=a&command=id",
            "zzclawterm://connect/ssh?host=%00example.com",
            "zzclawterm://connect/ssh?host=%GG",
            "zzclawterm://connect/ssh?host=example.com%2Fother",
            "zzclawterm://connect/rdp?host=example.com",
            "zzclawterm://connect/telnet?host=a&username=user",
        ] {
            assert!(parse_deep_link(link).is_err(), "accepted {link}");
        }
    }

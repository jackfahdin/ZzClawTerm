...
pub fn codex_initialize_request(id: u64, version: &str) -> Value {
    json!({
        ...
        "id": id,
        "params": {
            "clientInfo": { "name": "zzclawterm", "title": "ZzClawTerm", "version": version },
...
pub fn codex_decline_server_request(id: u64, method: &str) -> Value {
    let result = match method {
        ...
        "item/tool/call" => json!({
            "success": false,
            "contentItems": [{ "type": "inputText", "text": "Dynamic tools are disabled; use the configured ZzClawTerm MCP server." }]
        }),
...
mod tests {
    ...
    fn thread_params_are_read_only_and_use_ephemeral_mcp_environment() {
        let mcp = CodexMcpConfig {
            command: "zzclawterm-mcp".into(),
            env: HashMap::from([("ZZCLAWTERM_MCP_EPHEMERAL".into(), "1".into())]),
        };
        let request = codex_thread_start_request(2, Some("gpt-5-codex"), false, &mcp);
        let params = &request["params"];
        assert_eq!(params["sandbox"], "read-only");
        assert_eq!(
            params["config"]["mcp_servers"]["zzclawterm"]["required"],
            true
        );
        assert_eq!(
            params["config"]["mcp_servers"]["zzclawterm"]["env"]["ZZCLAWTERM_MCP_EPHEMERAL"],
            "1"
        );
    }

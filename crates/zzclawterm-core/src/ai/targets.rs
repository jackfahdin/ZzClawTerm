...
pub(super) fn user_input_with_target_contexts(request: &AiChatRequest) -> String {
    let mut result = format!(
        "<zzclawterm-user-task untrusted=\"true\" bytes=\"{}\">\n{}\n</zzclawterm-user-task>",
        request.user_input.len(),
        request.user_input
    );

    if !request.targets.is_empty() {
        result.push_str("\n\n<zzclawterm-terminal-targets untrusted=\"true\">\n");
        for target in &request.targets {
        ...
        result.push_str("</zzclawterm-terminal-targets>\n");
    }

    if !request.target_contexts.is_empty() {
        result.push_str("\n<zzclawterm-target-contexts untrusted=\"true\">\n");
        ...
        result.push_str("</zzclawterm-target-contexts>\n");
    }
    if !request.attachments.is_empty() {
        result.push_str("\n<zzclawterm-attachments untrusted=\"true\">\n");
        for attachment in &request.attachments {
            result.push_str(&format!(
                "- id={:?} name={:?} mime={:?} size={:?}\n",
        ...
        result.push_str("</zzclawterm-attachments>\n");
    }
    result
}

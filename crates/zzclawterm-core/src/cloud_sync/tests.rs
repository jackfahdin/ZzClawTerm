...
fn cloud_sync_debug_output_redacts_all_secret_values() {
    ...
    let options = LocalCloudSyncOptions {
        ...
        remote_root: "zzclawterm".to_string(),
...
impl ConnectionStore {
    ...
    fn db_path(&self) -> PathBuf {
        self.config_dir.join("zzclawterm.redb")
    }
}
...
fn remote_path_joins_without_duplicate_slashes() {
    assert_eq!(
        remote_path("zzclawterm", "sync/latest.redb"),
        "zzclawterm/sync/latest.redb"
    );
    assert_eq!(
        remote_path("/zzclawterm/", "/sync/latest.redb"),
        "zzclawterm/sync/latest.redb"
...
fn drive_remote_segments_trim_root_and_child_paths() {
    ...
    assert_eq!(
        drive_remote_segments("", "zzclawterm//sync/latest.redb"),
        vec!["zzclawterm", "sync", "latest.redb"]
    );
}
...
fn s3_signed_request_uses_path_style_url_and_headers() {
    let settings = S3SyncSettings {
        ...
        bucket: "zzclawterm-sync".to_string(),
    ...
    assert_eq!(
        request.url,
        "https://s3.example.com/zzclawterm-sync/profiles/default/zzclawterm/sync/latest%20redb"
    );
    assert_eq!(
...
fn s3_signed_request_supports_virtual_host_style() {
    ...
    assert_eq!(
        request.url,
        "https://zzclawterm.objects.example.com/base/sync/current.redb.enc"
    );
    assert_eq!(
        request.headers.get("host").map(String::as_str),
        Some("zzclawterm.objects.example.com")
    );
...
fn s3_signed_list_request_sorts_and_encodes_query_parameters() {
    ...
    let query = BTreeMap::from([
        (
            "prefix".to_string(),
            "zzclawterm/sync snapshots/".to_string(),
        ),
    ...
    assert_eq!(
        request.url,
        "https://s3.example.com/bucket?continuation-token=next%2B%2F%3D&list-type=2&prefix=zzclawterm%2Fsync%20snapshots%2F"
    );
    assert!(
...
fn local_cloud_sync_push_and_forced_pull_round_trip() {
    ...
    assert!(remote_dir.join("zzclawterm/sync/current.redb.enc").exists());
    assert!(remote_dir.join("zzclawterm/sync/latest.redb").exists());
...
fn cloud_sync_algorithm_uses_remote_backend_abstraction() {
    ...
    assert!(
        remote
            .read_if_exists("zzclawterm/sync/latest.redb")
            .expect("read pointer")
            .is_some()
    );
...
fn snippet_remote_codec_matches_legacy_blob_layout_and_syncs() {
    ...
    assert_eq!(
        snippet_remote_path(&snippet_remote_filename("zzclawterm/sync/latest.redb")).as_deref(),
        Some("zzclawterm/sync/latest.redb")
    );
    assert_eq!(
...
fn gitee_http_backend_fetches_raw_filename_with_access_token() {
    ...
    let content = backend
        .fetch_blob(&snippet_remote_filename("zzclawterm/sync/latest.redb"))
        .expect("fetch blob")
        .expect("blob");
    ...
    assert!(requests[0].url.contains("/gists/gist-1/raw/zzclawterm-"));
}

#[test]
fn github_gist_http_backend_fetches_raw_url_for_truncated_file() {
    let filename = snippet_remote_filename("zzclawterm/sync/current.redb.enc");
...
fn snippet_patch_bodies_match_gitee_and_github_shapes() {
    let mut files = BTreeMap::new();
    files.insert("zzclawterm-a.blob".to_string(), Some("payload".to_string()));
    files.insert("zzclawterm-b.blob".to_string(), None);

    let gitee = gitee_snippet_patch_body("token", files.clone());
    assert_eq!(gitee["access_token"], "token");
    assert_eq!(gitee["files"]["zzclawterm-a.blob"]["content"], "payload");
    assert!(gitee["files"]["zzclawterm-b.blob"].is_null());

    let github = github_gist_patch_body(files);
    assert!(github.get("access_token").is_none());
    assert_eq!(github["files"]["zzclawterm-a.blob"]["content"], "payload");
    assert!(github["files"]["zzclawterm-b.blob"].is_null());
}
...
fn unique_temp_dir(name: &str) -> crate::test_support::TestTempDir {
    let dir = crate::test_support::TestTempDir::new(&format!("zzclawterm-cloud-sync-{name}"));

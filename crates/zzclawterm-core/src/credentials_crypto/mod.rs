...
const WRAPPING_KEY_PREFIX: &[u8] = b"zzclawterm-key-wrap-v1:";
                ...
                decrypt_bytes(&raw, &legacy_key)
                    .map_err(|legacy_error| {
                        CredentialCryptoError::Decrypt(format!(
                            "ZzClawTerm key prefix failed ({new_error}); legacy Dragonfly key prefix failed ({legacy_error})"
                        ))
                    })
        ...
        match unwrap_master_key_bytes(raw, &wrapping_key) {
            ...
            Err(new_error) => {
                ...
                unwrap_master_key_bytes(raw, &legacy_wrapping_key)
                    ...
                    .map_err(|legacy_error| {
                        CredentialCryptoError::Decrypt(format!(
                            "unwrap master.key: ZzClawTerm key prefix failed ({new_error}); legacy Dragonfly key prefix failed ({legacy_error})"
                        ))
                    })
            }
        }

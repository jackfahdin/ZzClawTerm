...
pub enum PortableSnapshotError {
    ...
    Encrypt(String),
    #[error(
        "cloud snapshot decryption failed: ZzClawTerm key prefix failed ({new_error}); legacy Dragonfly key prefix failed ({legacy_error})"
    )]
    Decrypt {
        new_error: String,
        legacy_error: String,
...
pub fn encode_compressed_snapshot_payload(
    ...
    zip.write_all(
        br#"{"format":"zzclawterm-portable-snapshot-zip","version":1,"payload":"snapshot.redb"}"#,
    )?;

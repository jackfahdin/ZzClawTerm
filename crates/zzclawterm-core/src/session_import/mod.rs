...
//! SecureCRT (.xml), FinalShell conn directories, ZzClawTerm JSON files, Electerm
...
pub fn prepare_session_import(
    ...
    let prepared = if lower.ends_with(".xts") {
        ...
        return Err(AppError::Config(
            "Unsupported file format. Please use .xts (Xshell), .mxtsessions (MobaXterm), .sessions (WindTerm), .xml (SecureCRT), .json (ZzClawTerm JSON or Electerm bookmarks), or a FinalShell conn directory."
                .to_string(),
        ));
    };

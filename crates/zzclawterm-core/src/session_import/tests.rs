...
use super::{
    ...
    zzclawterm_json::parse_zzclawterm_json_content,
};
...
#[test]
fn zzclawterm_json_sample_import_prepares_supported_shapes() {
    let prepared = parse_zzclawterm_json_content(SAMPLE_JSON).expect("parse sample");
...
#[test]
fn zzclawterm_json_rejects_duplicate_password_refs() {
    let json = r#"
{
  "version": 1,
  "passwords": [
...
"#;
    ...
    let error = parse_zzclawterm_json_content(json).unwrap_err();
    assert!(error.to_string().contains("Duplicate password ref"));
}

#[test]
fn zzclawterm_json_rejects_missing_password_refs() {
    let json = r#"
{
  "version": 1,
  "sessions": [
...
#[test]
fn zzclawterm_json_rejects_invalid_ports() {
    let json = r#"
{
  "version": 1,
  "sessions": [
...
#[test]
fn zzclawterm_json_retains_pipeline_shared_icons_and_future_connection_fields() {
    let prepared = parse_zzclawterm_json_content(r#"{"version":1,"custom_icons":[{"id":"custom-icon-test","name":"test","data_url":"data:image/png;base64,AA==","created_at_ms":1,"updated_at_ms":1}],"sessions":[{"name":"test","type":"ssh","host":"example.com","sftp":{"pipeline_depth":16},"icon":"custom-icon-test","future":{"enabled":true}}]}"#).unwrap();

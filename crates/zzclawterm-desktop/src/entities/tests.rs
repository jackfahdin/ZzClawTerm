...
fn startup_restore_store_tracks_queue_admission() {
    let mut store = StartupRestoreStore::default();
    store.set_queue(vec![
        zzclawterm_core::RestorableOpenTab::with_leaf_root("one", "Local", None, None, None),
        zzclawterm_core::RestorableOpenTab::with_leaf_root("two", "Local", None, None, None),
    ]);
...
fn overlay_store_owns_quick_switch_state(cx: &mut gpui::TestAppContext) {
    ...
    cx.update(|window, cx| {
        let command_state = cx.new(|cx| zzclawterm_ui::ZzClawCommandState::new(window, cx));

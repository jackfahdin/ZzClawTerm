...
pub(in crate::features) fn run_ai_ask_job(
    ...
    if request.agent_kind == AiAgentKind::Codex {
        if settings.codex.tool_integration_mode.as_deref() != Some("zzclawterm_mcp") {
            return Err("Codex requires strict ZzClawTerm MCP tool integration".to_string());
        }
        let persistent = settings.codex.thread_mode == zzclawterm_core::CodexThreadMode::Persistent;
                                    ...
                                    .filter(|metadata| {
                                        metadata.backend == AiBackendKind::Codex
                                            && metadata.codex_terminal_tools_version
                                                == Some(
                                                    zzclawterm_core::ai::CODEX_TERMINAL_TOOLS_VERSION,
                                                )
                                    })
        ...
        let credential = mcp_credential.ok_or_else(|| {
            "Codex requires a request-scoped ZzClawTerm MCP credential".to_string()
        })?;
        ...
        if settings.record_history && persistent {
            ...
            let metadata = AiSessionBackendMetadata {
                ...
                codex_terminal_tools_version: Some(
                    zzclawterm_core::ai::CODEX_TERMINAL_TOOLS_VERSION,
                ),
            };
    ...
    if request.agent_kind == AiAgentKind::ClaudeCode {
        if settings.claude_code.tool_integration_mode.as_deref() != Some("zzclawterm_mcp") {
            return Err("Claude Code requires strict ZzClawTerm MCP tool integration".to_string());
        }
        ...
        let credential = mcp_credential.ok_or_else(|| {
            "Claude Code requires a request-scoped ZzClawTerm MCP credential".to_string()
        })?;
...
fn record_external_agent_audit(
    ...
    let audit = AppendAiAuditRequest {
        ...
        client: None,
        capability: Some("zzclawterm_mcp".to_string()),
...
mod tests {
    use zzclawterm_core::{
        AiAction, AiChatRequest, AiMode, AiSettings, AiTerminalTarget, AiToolCall,
    };

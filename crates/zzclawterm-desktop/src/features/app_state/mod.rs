...
use zzclawterm_store::{StoreBlockingClient, StoreUiClient};
...
impl gpui::EventEmitter<NotesCatalogEvent> for ZzClawTermApp {}

/// Requests that the application shell start its orderly shutdown sequence.
///
/// The shell owns persistence and worker shutdown, while `ZzClawTermApp` owns the

...
use zzclawterm_transport::{RemoteGpuOverview, RemoteNpuOverview, RemoteStats};
use zzclawterm_ui::{
    ZzClawInputEvent, ZzClawInputState, ZzClawSelectEvent, ZzClawSelectOption, ZzClawSelectState,
};
...
pub(in crate::features) struct StartWorkspaceFeatureState {
    ...
    group_select: Entity<ZzClawSelectState>,
...
impl StartWorkspaceFeatureState {
    pub fn new(
        groups: &[Group],
        settings: &zzclawterm_core::AppSettingsSummary,
        cx: &mut Context<ZzClawTermApp>,
    ) -> Self {
        let search_field = cx.new(|cx| {
            ZzClawInputState::new(cx, String::new())
                .placeholder(rust_i18n::t!("assets.searchPlaceholder"))
        });
        let search_subscription = cx.subscribe(
            &search_field,
            |app: &mut ZzClawTermApp, _, event: &ZzClawInputEvent, cx| {
                if let ZzClawInputEvent::Changed(value) | ZzClawInputEvent::Submitted(value) = event
                {
        ...
        options.extend(
            build_group_options(groups, &rust_i18n::t!("assets.title"))
                .into_iter()
                .map(|option| {
                    ZzClawSelectOption::new(option.group.id, option.path)
                        .search_text(option.search_text)
                }),
        );
        let group_select = cx.new(|cx| {
            ZzClawSelectState::new(cx, options, Some(String::new()))
        ...
        let group_subscription = cx.subscribe(
            &group_select,
            |app: &mut ZzClawTermApp, _, event: &ZzClawSelectEvent, cx| {
                let ZzClawSelectEvent::Changed(value) = event;
    ...
    pub fn group_select(&self) -> Entity<ZzClawSelectState> {

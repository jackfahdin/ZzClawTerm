...
use super::sources::{
    ...
    parse_windterm_quickbar, parse_xshell_quick_buttons_content, parse_zzclawterm_import,
};

#[test]
fn imports_config_and_array_shapes() {
    let config = parse_zzclawterm_import(
    ...
    let config = parse_zzclawterm_import(r#"[{"label":"Pwd","command":"pwd"}]"#)
...
fn merge_import_updates_commands_and_creates_named_categories() {
    ...
    let import_config = parse_zzclawterm_import(
    ...
    let import_config =
        parse_zzclawterm_import(r#"[{"id":"c1","label":"List all","command":"ls -la"}]"#)
            .expect("json parses");

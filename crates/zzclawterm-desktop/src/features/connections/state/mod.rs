...
use zzclawterm_core::{AppSettingsSummary, ConnectionType, Group, SavedConnection};
use zzclawterm_ui::{ChildWindowSlot, ZzClawWindowHandle};
...
use zzclawterm_ui::{
    ZzClawInputEvent, ZzClawInputState, ZzClawNumberInputEvent, ZzClawNumberInputOptions,
    ZzClawNumberInputState,
};
...
struct ConnectionEditorFeatureState {
    ...
    fields: HashMap<ConnectionEditorField, Entity<ZzClawInputState>>,
    number_fields: HashMap<ConnectionEditorField, Entity<ZzClawNumberInputState>>,
    field_subscriptions: Vec<Subscription>,
    number_field_subscriptions: Vec<Subscription>,
    forwarding_endpoint_fields: HashMap<(usize, ConnectionEditorField), Entity<ZzClawInputState>>,
...
struct ConnectionGroupEditorFeatureState {
    ...
    field: Option<Entity<ZzClawInputState>>,
...
impl ConnectionFeatureState {
    pub fn new(
        ...
        let search_field =
            cx.new(|cx| ZzClawInputState::new(cx, String::new()).placeholder(filter_placeholder));
        // The field owns the text; the panel only needs to know when it changed.
        let search_subscription = cx.subscribe(
            &search_field,
            |app: &mut ZzClawTermApp, _, event: &ZzClawInputEvent, cx| match event {
    ...
    pub fn editor_fields(&self) -> &HashMap<ConnectionEditorField, Entity<ZzClawInputState>> {
        &self.editor.fields
    }

    pub fn editor_number_fields(
        &self,
    ) -> &HashMap<ConnectionEditorField, Entity<ZzClawNumberInputState>> {
        &self.editor.number_fields
    }

    pub fn editor_forwarding_endpoint_fields(
        &self,
    ) -> &HashMap<(usize, ConnectionEditorField), Entity<ZzClawInputState>> {
    ...
    pub fn build_editor_fields(&mut self, cx: &mut Context<ZzClawTermApp>) {
        ...
        for (field, value, masked, placeholder) in editor_field_seeds(draft) {
            ...
            if let Some(options) = connection_editor_number_options(field) {
                let entity = cx.new(|cx| {
                    ZzClawNumberInputState::new(cx, value, options).placeholder(placeholder)
                });
                let subscription = cx.subscribe(
                    &entity,
                    move |app: &mut ZzClawTermApp, _, event, cx| match event {
                        ZzClawNumberInputEvent::Changed(text)
                        | ZzClawNumberInputEvent::Submitted(text) => {
                            app.apply_connection_editor_field_text(field, text.clone(), cx);
                        }
                        ZzClawNumberInputEvent::Stepped(_) => {}
                    },
                );
    ...
    pub fn ensure_editor_forwarding_endpoint_fields(&mut self, cx: &mut Context<ZzClawTermApp>) {
        let seeds = self
            .editor
            .draft
            .as_ref()
        ...
        for (index, field, value, placeholder) in seeds {
            ...
            let entity = cx.new(|cx| ZzClawInputState::new(cx, seed).placeholder(placeholder));
            let subscription = cx.subscribe(
                &entity,
                move |app: &mut ZzClawTermApp, _, event, cx| match event {
                    ZzClawInputEvent::Changed(text) | ZzClawInputEvent::Submitted(text) => {
    ...
    pub fn rebuild_editor_forwarding_endpoint_fields(&mut self, cx: &mut Context<ZzClawTermApp>) {
        self.editor.forwarding_endpoint_fields.clear();
        self.editor.forwarding_endpoint_field_subscriptions.clear();
        self.ensure_editor_forwarding_endpoint_fields(cx);
    }
    ...
    pub fn set_editor_agent_preview(
        &mut self,
        generation: u64,
        preview: zzclawterm_transport::SshAgentIdentityPreviewResponse,
    ) -> bool {
    ...
    pub(in crate::features) fn editor_window_handle(&self) -> Option<ZzClawWindowHandle> {
    ...
    pub fn group_editor_field(&self) -> Option<Entity<ZzClawInputState>> {
    ...
    pub fn build_group_editor_field(
        ...
        let entity =
            cx.new(|cx| ZzClawInputState::new(cx, draft.name.clone()).placeholder(placeholder));
        let subscription = cx.subscribe(
            &entity,
            |app: &mut ZzClawTermApp, _, event, cx| match event {
...
impl ConnectionEditorFeatureState {
    ...
    pub fn window_handle(&self) -> Option<ZzClawWindowHandle> {
        self.window.handle()
    }
    ...
    pub fn attach_window(&mut self, window: ZzClawWindowHandle) {
...
fn connection_editor_number_options(
    field: ConnectionEditorField,
) -> Option<ZzClawNumberInputOptions> {
    match field {
        ...
        ConnectionEditorField::TelnetAutoLoginMaxRetries => Some(
            ZzClawNumberInputOptions::default()
                .range(0.0, 10.0)
                .step(1.0),
        ),
        ...
        ConnectionEditorField::RdpReconnectAttempts => Some(
            ZzClawNumberInputOptions::default()
                .range(0.0, 20.0)
                .step(1.0),
        ),

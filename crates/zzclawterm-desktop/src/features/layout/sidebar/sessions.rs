...
use zzclawterm_core::{RuntimeMode, truncate_preview};
use zzclawterm_transport::SessionInfo;
use zzclawterm_ui::{ZzClawDropdownMenu, ZzClawMenuItem, ZzClawScrollable};
...
impl ZzClawTermApp {
    ...
    pub(in crate::features) fn active_session_row(
        ...
        let menu = ZzClawDropdownMenu::new(format!("active-session-more-{}", session.id))
            ...
            .min_width(px(160.))
            .items([
                ZzClawMenuItem::action(reconnect_label)
                ...
                ZzClawMenuItem::action(disconnect_label)

    ...
    pub fn start(
        ...
        if let Some(store) = discovery.as_ref() {
            ...
            store.write(&DiscoveryDocument {
                version: zzclawterm_mcp_protocol::PROTOCOL_VERSION,
                pid: std::process::id(),
        ...
        let thread = spawn_joinable("zzclawterm-mcp-host", move || {
...
async fn handle_execute(
        ...
        .is_err()
    {
        ...
        return rpc_error(
            id,
            "host_unavailable",
            "ZzClawTerm MCP Host is unavailable.",
        );
    }
    let result = tokio::select! {
        ...
        result = tokio::time::timeout(REQUEST_REPLY_TIMEOUT, result) => match result {
            Ok(Ok(result)) => result,
            Ok(Err(_)) => Err(rpc_failure("host_unavailable", "ZzClawTerm closed the MCP request.")),
            Err(_) => Err(rpc_failure("host_timeout", "ZzClawTerm MCP Host request timed out.")),
        }
    };
    cancellation.cancel();
    cancellations.lock().await.remove(&key);

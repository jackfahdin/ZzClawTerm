...
use zzclawterm_ui::{
    ...
    ZzClawDocumentEditorEvent, ZzClawDocumentEditorState, ZzClawDropdownMenu, ZzClawInput,
    ZzClawInputEvent, ZzClawInputShell, ZzClawInputState, ZzClawMenuItem, ZzClawScrollable as _,
    ZzClawWindowHandle, zzclaw_root,
};
...
impl NoteEditorWindow {
    fn new(
        ...
        let editor = cx.new(|cx| ZzClawDocumentEditorState::new(window, cx, note.markdown.clone()));
        let title = cx.new(|cx| ZzClawInputState::new(cx, note.title.clone()).max_chars(Some(120)));
        let editor_subscription = cx.subscribe(
            &editor,
            |this, _, event: &ZzClawDocumentEditorEvent, cx| match event {
                ZzClawDocumentEditorEvent::Changed(value) => {
                    this.note.markdown = value.clone();
                    this.mark_dirty(cx);
                }
                ZzClawDocumentEditorEvent::Blurred(value) => {
        ...
        let title_subscription =
            cx.subscribe(
                &title,
                |this, _, event: &ZzClawInputEvent, cx| match event {
                    ZzClawInputEvent::Changed(value) => {
                        this.note.title = value.clone();
                        this.mark_dirty(cx);
                    }
                    ZzClawInputEvent::Blurred(value) | ZzClawInputEvent::Submitted(value) => {
            ...
            app.submit_store_request(
                ...
                move |app, event, cx| match event.outcome {
                    Ok(note) => {
                        ...
                        app.notes
                            .upsert_note(zzclawterm_core::NoteSummary::from(&note));
    ...
    fn open_link_dialog(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let label = cx.new(|cx| ZzClawInputState::new(cx, label_seed.clone()));
        let url = cx.new(|cx| {
            ZzClawInputState::new(cx, "https://").placeholder(t!("notes.linkUrlPlaceholder"))
        });
            ...
            .content(
                div()
                    ...
                    .child(
                        div()
                            .h(px(34.))
                            .child(ZzClawInputShell::new("note-link-label", &label)),
                    )
                    .child(
                        div()
                            .h(px(34.))
                            .child(ZzClawInputShell::new("note-link-url", &url)),
                    ),
            )
        ...
        if matches!(mode, NoteViewMode::Source | NoteViewMode::Split) {
            body = body.child(
                div()
                    ...
                    .child(ZzClawDocumentEditor::new(&editor)),
            );
        }
                ...
                div()
                    ...
                    .child(
                        div()
                            .w(px(220.))
                            .h(px(30.))
                            .child(ZzClawInput::new(&self.title)),
                    )
                    .child(toolbar)
                    .child(div().flex_1())
                    .child(
...
fn open_note_editor_window(app: Entity<ZzClawTermApp>, note: NoteDocument, cx: &mut App) {
    ...
    let result: anyhow::Result<ZzClawWindowHandle> = cx.open_window(options, move |window, cx| {
        ...
        cx.new(|cx| zzclaw_root(view, window, cx))
    });

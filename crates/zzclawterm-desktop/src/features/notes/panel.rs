...
use rust_i18n::t;
use zzclawterm_core::{NoteFolder, NoteNodeKind};
use zzclawterm_ui::{
    ZzClawButton, ZzClawButtonVariant, ZzClawContextMenu, ZzClawDropdownMenu, ZzClawInputEvent,
    ZzClawInputShell, ZzClawInputState, ZzClawMenuItem, ZzClawScrollable as _, ZzClawSearchInput,
};

use crate::features::{ZzClawTermApp, view_widgets::mono_icon};
...
pub(in crate::features) struct NotesPanel {
    ...
    rename: Entity<ZzClawInputState>,
...
impl NotesPanel {
    pub(in crate::features) fn new(app: WeakEntity<ZzClawTermApp>, cx: &mut Context<Self>) -> Self {
        let search = cx.new(|cx| {
            ZzClawInputState::new(cx, String::new()).placeholder(t!("notes.searchPlaceholder"))
        });
        let rename = cx.new(|cx| ZzClawInputState::new(cx, String::new()).max_chars(Some(120)));
        let app_subscription = cx.observe(
            &app.upgrade().expect("Notes panel is created with its app"),
            |_, _, cx| cx.notify(),
        );
        let search_subscription =
            cx.subscribe(&search, |_, _, _: &ZzClawInputEvent, cx| cx.notify());
        let rename_subscription = cx.subscribe(&rename, |this, _, event: &ZzClawInputEvent, cx| {
            if let ZzClawInputEvent::Submitted(value) | ZzClawInputEvent::Blurred(value) = event {
    ...
    fn with_app(
        &self,
        cx: &mut Context<Self>,
        f: impl FnOnce(&mut ZzClawTermApp, &mut Context<ZzClawTermApp>),
    ) {
...
impl Render for NotesPanel {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let mut search_input = ZzClawSearchInput::new("notes-search", &search_state);
        if !search.is_empty() {
        ...
        if loading && catalog_empty {
            ...
            root = root.child(
                ZzClawContextMenu::new_dynamic(tree, move |_, cx| {
...
fn notes_more_menu(app: WeakEntity<ZzClawTermApp>) -> ZzClawDropdownMenu {
    ...
    ZzClawDropdownMenu::new("notes-more")
        ...
        .items([
            ...
            ZzClawMenuItem::action(t!("notes.collapseAll").to_string()).on_click(
                move |_, _, cx| {
            ...
            ZzClawMenuItem::action(t!("common.refresh").to_string())
...
fn toolbar_button(
    ...
    palette: crate::theme::ThemePalette,
    app: WeakEntity<ZzClawTermApp>,
    action: impl Fn(&mut ZzClawTermApp, &mut Context<ZzClawTermApp>) + 'static,
) -> gpui::AnyElement {
    ...
    div()
        ...
        .tooltip(move |window, cx| {
            zzclawterm_ui::ZzClawTooltip::new(tooltip.clone()).build(window, cx)
        })
        .on_click(move |_, _, cx| {
            if let Some(app) = app.upgrade() {
        ...
        return div()
            ...
            .child(
                div()
                    .w_full()
                    .h(px(24.))
                    .child(ZzClawInputShell::new("notes-inline-rename", &panel.rename)),
            )
            .into_any_element();
    }
...
fn note_context_menu_items(
    ...
    let Some(target_id) = target_id else {
        ...
        return vec![
            ZzClawMenuItem::action(t!("notes.newNote").to_string())
            ...
            ZzClawMenuItem::action(t!("notes.newFolder").to_string())
    ...
    if kind == NoteNodeKind::Note {
        ...
        items.push(
            ZzClawMenuItem::action(t!("notes.open").to_string())
    ...
    items.push(
        ZzClawMenuItem::action(t!("notes.rename").to_string())
    ...
    move_items.push(
        ZzClawMenuItem::action(t!("notes.root").to_string()).on_click(move |_, _, cx| {
    ...
    for folder in folder_targets {
        ...
        move_items.push(
            ZzClawMenuItem::action(folder.name)
    ...
    items.push(
        ZzClawMenuItem::submenu(t!("notes.moveTo").to_string(), move_items)
    ...
    items.push(
        ZzClawMenuItem::action(t!("common.delete").to_string())
...
mod tests {
    ...
    fn row_drop_targets_match_tauri_parent_rules() {
        let folder = NoteTreeRow {
            id: "folder".into(),
            kind: zzclawterm_core::NoteNodeKind::Folder,
            parent_id: None,
            name: "Folder".into(),
            depth: 0,
            expanded: false,
        ...
        let note = NoteTreeRow {
            id: "note".into(),
            kind: zzclawterm_core::NoteNodeKind::Note,
    ...
    fn context_menu_items_match_blank_folder_and_note_targets() {
        let test_dir = TestConfigDir::new("zzclawterm-notes-panel");

...
use zzclawterm_core::{NoteDocument, NoteNodeKind, NoteSummary};
use zzclawterm_store::{StoreDomain, store_request};
use zzclawterm_ui::{ZzClawConfirmDialog, ZzClawDialogFooter, ZzClawDialogWindowExt as _};

use crate::{
    features::{ZzClawTermApp, notes::NotesCatalogEvent},
    models::NavItem,
};
...
mod tests {
    ...
    use crate::features::{ZzClawTermApp, notes::NotesCatalogEvent};
    use crate::models::NavItem;

    fn unique_test_dir() -> PathBuf {
        std::env::temp_dir().join(format!(
            "zzclawterm-notes-runtime-{}-{}",
            std::process::id(),
            uuid()
        ))
    }

    fn test_app(cx: &mut TestAppContext) -> gpui::Entity<ZzClawTermApp> {
    ...
    impl NotesEventSink {
        fn new(app: &Entity<ZzClawTermApp>, cx: &mut Context<Self>) -> Self {

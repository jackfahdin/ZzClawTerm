...
impl ConnectionPanel {
    pub(in crate::features) fn new(app: WeakEntity<ZzClawTermApp>, focus: FocusHandle) -> Self {
        Self {
            app,
            snapshot: None,
            focus,
    ...
    /// `ZzClawTermApp` first, then defer the snapshot flush until the current GPUI effect
    ...
    pub(in crate::features::pages::connections) fn app_handle(&self) -> WeakEntity<ZzClawTermApp> {
        self.app.clone()
    }
}
...
impl Render for ConnectionPanel {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // Renders with **zero** `ZzClawTermApp` access, diagnostics included: GPUI
...
mod tests {
    ...
    use zzclawterm_core::{AppRuntime, Group, RuntimeMode, SavedConnection};
    ...
    type Mutation = Box<dyn Fn(&mut ZzClawTermApp)>;

    fn select_only(app: &mut ZzClawTermApp, id: &str) {
    ...
    fn rows_built(app: &Entity<ZzClawTermApp>, cx: &mut gpui::App) -> usize {
    ...
    fn interaction_state_changes_reach_the_panel() {
        ...
        for (label, mutate) in [
            (
                "selection",
                Box::new(|app: &mut ZzClawTermApp| select_only(app, "a")) as Mutation,
            ),
            (
                "keyboard active",
    ...
    fn search_input_changes_reach_the_snapshot_after_the_subscription_runs() {
        ...
        vcx.update(|_, cx| {
            search_field.update(cx, |_, cx| {
                cx.emit(ZzClawInputEvent::Changed("Alpha".to_string()));
            });
            assert!(
                app.read(cx)
                    .connection_panel
    ...
    fn every_selection_mutation_moves_the_arc_while_a_snapshot_holds_it() {
        ...
        let mutations: Vec<(&str, Mutation)> = vec![
            (
                ...
                Box::new(|app: &mut ZzClawTermApp| select_only(app, "a")),
            ),
            (
                "additive select",
                Box::new(|app: &mut ZzClawTermApp| {

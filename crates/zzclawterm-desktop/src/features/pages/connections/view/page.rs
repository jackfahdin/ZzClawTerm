...
use zzclawterm_ui::{
    ZzClawContextMenu, ZzClawDropdownMenu, ZzClawScrollable, ZzClawScrollbarAxis, ZzClawSearchInput,
};
...
/// The panel body.
///
/// Takes no `ZzClawTermApp`. GPUI records every entity read during a draw, so an app
...
pub(in crate::features::pages::connections) fn connections_panel(
    ...
    // whose content fits.
    let list = list.scrollbar(&list_scroll, ZzClawScrollbarAxis::Both);
    ...
    let list = ZzClawContextMenu::new_dynamic(
...
fn connections_search_bar(
    ...
    let more_menu = ZzClawDropdownMenu::new("connections-more")

...
use zzclawterm_ui::{
    ZzClawHoverCard, ZzClawIconButton, ZzClawInput, ZzClawPopoverAlign, ZzClawPopoverPlacement,
};
...
///
/// `ZzClawHoverCard` keeps this content open while the pointer crosses from the
        ...
        for (index, row) in self.rows.iter().enumerate() {
            ...
            let copy = row.copy_value.clone().map(|copy_value| {
                ...
                div()
                    ...
                    .justify_center()
                    .child(
                        ZzClawIconButton::new(format!("{copy_id}-button"), "icons/copy.svg")
                            .icon_size(px(12.))

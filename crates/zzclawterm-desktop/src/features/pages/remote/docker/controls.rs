...
use zzclawterm_transport::RemoteDockerOverview;
use zzclawterm_ui::{ZzClawTabItem, ZzClawTabs};
...
pub(in crate::features::pages::remote) fn docker_tab_bar(
    ...
    bar = bar.child(
        div().min_w_0().flex_1().child(
            ZzClawTabs::new("docker-tabs")
                .items(
                    visible_tabs
                        .iter()
                        .map(|(_, label)| ZzClawTabItem::new((*label).clone())),
                )

...
//! `ZzClawTermApp.remote_ops` stays the single authoritative owner of every pane's data,
...
//!
//! These are held by `ZzClawTermApp` for its whole life, so a hidden panel is **not**
...
/// touching `ZzClawTermApp` at all -- and that matters beyond tidiness, because GPUI records
...
pub(in crate::features) struct RemoteMonitorPanel {
    ...
    /// Weak on purpose. `ZzClawTermApp` owns this entity, so a strong handle back would
...
impl RemoteMonitorPanel {
    ...
    fn new(kind: RemoteMonitorKind, app: WeakEntity<ZzClawTermApp>) -> Self {
        Self {
            kind,
            app,
            clock: None,
...
impl Render for RemoteMonitorPanel {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // Snapshot kinds render with **zero** `ZzClawTermApp` access, diagnostics included:
...
/// The five panel entities, as one field on `ZzClawTermApp`.
...
impl RemotePanels {
    pub(in crate::features) fn new(
        ...
        let panel = |kind: RemoteMonitorKind, cx: &mut Context<ZzClawTermApp>| {
            let app = app.clone();
            ...
            RemoteMonitorKind::Processes => {
                ...
                let nice = state.selected_pid.map(|pid| {
                    self.number_input(
                        ...
                        ZzClawNumberInputOptions::default().range(-20.0, 19.0),
                        cx,
                    )
                });
                RemoteMonitorData::Processes {
...
mod tests {
    ...
    fn every_kind_gets_its_own_entity() {
        let test_dir = TestConfigDir::new("zzclawterm-remote-panels");
    ...
    fn activate_ssh_session(app: &Entity<ZzClawTermApp>, cx: &mut TestAppContext) {
        cx.update_entity(app, |app, _| {
            app.session.register_session_metadata(
                "remote-panel-test",
                crate::models::SessionRuntimeMetadata {
                    ssh_config: Some(zzclawterm_transport::SshSessionConfig::default()),
    ...
    fn polling(app: &Entity<ZzClawTermApp>, cx: &mut TestAppContext) -> Vec<RemoteMonitorKind> {
        cx.update_entity(app, |app, cx| {
...
mod isolation_tests {
    ...
    use zzclawterm_core::{AiExecutionProfile, AppRuntime, RuntimeMode};
    use zzclawterm_transport::{
        LocalSessionConfig, RemoteGpuOverview, RemoteStats, SshSessionConfig,
    };
    ...
    fn paints(app: &gpui::Entity<ZzClawTermApp>, cx: &gpui::App) -> [usize; 3] {
        let count = |kind| {
            app.read(cx)
                .remote_panels
                .entity(kind)
    ...
    fn hosted<'a>(
        ...
        root: &Path,
    ) -> (gpui::Entity<ZzClawTermApp>, &'a mut VisualTestContext) {
        let app = app(cx, root);
        cx.update_entity(&app, |app, cx| {
            app.sync_component_theme(cx);
            let mut summary = app.settings.summary().clone();
    ...
    /// makes **zero** `ZzClawTermApp` accesses: GPUI records every entity read during a
    ...
    fn an_unrelated_app_notify_does_not_re_render_the_snapshot_panels() {
        let test_dir = TestConfigDir::new("zzclawterm-panel-isolation");
        let mut cx = TestAppContext::single();
    ...
    ///
    /// The panel holds the field's `Entity<ZzClawInputState>` from its snapshot, so reading
    ...
    fn remote_search_input_reaches_its_snapshot_after_the_subscription_runs() {
        ...
        vcx.update(|_, cx| {
            field.update(cx, |_, cx| {
                cx.emit(ZzClawInputEvent::Changed("py".to_string()));
            });
            assert_eq!(
                app.read(cx)
                    .remote_panels
    ...
    fn process_paints(app: &gpui::Entity<ZzClawTermApp>, cx: &gpui::App) -> usize {
        app.read(cx)
            .remote_panels
    ...
    fn process(pid: u32) -> zzclawterm_transport::RemoteProcess {
        zzclawterm_transport::RemoteProcess {
            pid,
            ppid: 1,
            user: "user".to_string(),
            state: "S".to_string(),
        ...
        for (label, mutate) in [
            (
                ...
                &(|app: &mut ZzClawTermApp| {
                    app.remote_ops.apply_process_search("sleep".to_string())
                }) as &dyn Fn(&mut ZzClawTermApp),
            ),
            ("sort", &|app: &mut ZzClawTermApp| {
                app.remote_ops
                    .toggle_process_sort(crate::models::RemoteProcessSortKey::Pid)
            }),
            ("selection", &|app: &mut ZzClawTermApp| {
                app.remote_ops.toggle_process_selection(3)
            }),
            ("nice input", &|app: &mut ZzClawTermApp| {
    ...
    fn docker_overview(containers: usize) -> zzclawterm_transport::RemoteDockerOverview {
        zzclawterm_transport::RemoteDockerOverview {
            available: true,
            compose_available: true,
            containers: (0..containers)
                .map(|index| zzclawterm_transport::DockerContainer {
                    id: format!("c{index}"),
                ...
                .collect(),
            images: (0..30)
                .map(|index| zzclawterm_transport::DockerImage {
                    id: format!("i{index}"),
    ...
    fn docker_paints(app: &gpui::Entity<ZzClawTermApp>, cx: &gpui::App) -> usize {
        app.read(cx)
            .remote_panels
    ...
    fn docker_interactions_update_only_the_docker_panel() {
        ...
        for (label, mutate) in [
            (
                "search",
                &(|app: &mut ZzClawTermApp| app.remote_ops.apply_docker_search("name1".to_string()))
                    as &dyn Fn(&mut ZzClawTermApp),
            ),
            ("tab", &|app: &mut ZzClawTermApp| {
                app.remote_ops.set_docker_tab(DockerTab::Images)
            }),
            ("container menu", &|app: &mut ZzClawTermApp| {
                app.remote_ops
                    .toggle_docker_container_menu("c0".to_string())
            }),
            ("header menu", &|app: &mut ZzClawTermApp| {
                app.remote_ops.toggle_docker_header_menu()
            }),
            ("compose project", &|app: &mut ZzClawTermApp| {
                app.remote_ops
    ...
    fn duplicate_docker_image_ids_keep_accessibility_nodes_unique() {
        let test_dir = TestConfigDir::new("zzclawterm-docker-image-a11y");
        let mut cx = TestAppContext::single();
        let (app, vcx) = hosted_docker(&mut cx, test_dir.path());
            ...
            app.update(cx, |app, cx| {
                app.remote_ops
                    .apply_docker_resource(DockerResource::Images(vec![
                        zzclawterm_transport::DockerImage {

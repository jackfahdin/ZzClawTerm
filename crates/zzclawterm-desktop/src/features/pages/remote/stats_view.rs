...
///
/// Takes no `ZzClawTermApp`. GPUI records every entity read during a view's render as a
...
fn stats_empty_panel(
    ...
    zzclawterm_ui::empty_panel_with_icon(text, palette, "icons/resources.svg")
}
...
fn gpu_empty_panel(
    ...
    zzclawterm_ui::empty_panel_with_icon(text, palette, "icons/gpu.svg")
}
...
fn npu_empty_panel(
    ...
    zzclawterm_ui::empty_panel_with_icon(text, palette, "icons/npu.svg")
}
...
pub(in crate::features::pages::remote) fn gpu_panel(
    ...
    let gpu_search_input = ZzClawSearchInput::new("remote.gpu.filter", &search).into_any_element();
    if !has_session {
        return div()
            .size_full()
            .bg(chrome.transparent_surface)
...
pub(in crate::features::pages::remote) fn npu_panel(
    ...
    let palette = chrome.palette;
    let npu_search_input = ZzClawSearchInput::new("remote.npu.filter", &search).into_any_element();
    if !has_session {
        return div()
            .size_full()
            .bg(chrome.transparent_surface)
...
#[cfg(test)]
mod tests {
    use zzclawterm_transport::{RemoteGpu, RemoteGpuOverview, RemoteNpu, RemoteNpuOverview};

...
use gpui::Context;
use zzclawterm_ui::ZzClawNumberInputOptions;

use crate::features::ZzClawTermApp;
use crate::features::text_inputs::{TextInputSetup, secret_input_setup};
use zzclawterm_core::RecordingRotationPolicy;
...
mod tests {
    ...
    use zzclawterm_core::{AppRuntime, CloudSyncState, RuntimeMode};
    ...
    fn hosted(cx: &mut TestAppContext, root: &Path) -> Entity<ZzClawTermApp> {
        let app = app(cx, root);
    ...
    fn every_settings_tab_builds_its_inputs_and_snapshot_handles() {
        let test_dir = TestConfigDir::new("zzclawterm-settings-inputs");
    ...
    fn flush(app: &Entity<ZzClawTermApp>, vcx: &mut VisualTestContext) {
        vcx.update(|_, cx| {
    ...
    #[track_caller]
    fn assert_released(app: &Entity<ZzClawTermApp>, vcx: &mut VisualTestContext, text: &[&str]) {
        flush(app, vcx);
        vcx.update(|_, cx| {
            let panel = app.read(cx).settings_panel.read(cx);
            let snapshot = panel.snapshot().expect("the panel has a snapshot");
    ...
    fn reveal_boundaries_build_the_inputs_their_rows_draw() {
        ...
        for kind in [AiActionListKind::Terminal, AiActionListKind::File] {
            ...
            assert_built(
                ...
                vcx,
                &[
                    &ZzClawTermApp::ai_action_text_input_id(
                        kind,
                        &action_id,
    ...
    ///
    /// A real keypress is forwarded as `ZzClawInputEvent::Changed` by the input
    ...
    fn webdav_keystroke_enables_the_flushed_panel_apply_button() {
        ...
        let _sink = cx.new(|cx| {
            let _subscription = cx.subscribe(&field, move |_, _, _: &ZzClawInputEvent, _| {
                probe.set(true);
            });
            EmitSink { _subscription }
        });
        cx.update_entity(&field, |_input, cx| {
            cx.emit(ZzClawInputEvent::Changed("https://dav.example.com".into()));
        });
        cx.run_until_parked();
    ...
    fn master_password_keystroke_enables_the_flushed_panel_apply_button() {
        ...
        cx.update_entity(&field, |_input, cx| {
            cx.emit(ZzClawInputEvent::Changed("new secret".into()));
        });
        cx.run_until_parked();
    ...
    fn successful_cloud_sync_connection_test_records_check_time_only() {
        ...
        cx.update_entity(&app, |app, _| {
            app.store_blocking_client()
                .request_fn(zzclawterm_store::StoreDomain::CloudSync, move |store| {
        ...
        cx.update_entity(&app, |app, _| {
            ...
            let persisted = app
                ...
                .request_fn(zzclawterm_store::StoreDomain::CloudSync, |store| {

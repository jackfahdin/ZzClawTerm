...
use crate::features::{
    ...
    FontResolutionStatus, ZzClawTermApp, normalize_font_family,
};
use crate::models::{
...
pub(in crate::features) struct SettingsSnapshot {
    ...
    pub text_inputs: HashMap<SharedString, Entity<zzclawterm_ui::ZzClawInputState>>,
    pub number_inputs: HashMap<SharedString, Entity<ZzClawNumberInputState>>,
    pub draft_open: bool,
...
pub(in crate::features) struct MasterPasswordPresentation {
    pub enabled: bool,
    pub draft: zzclawterm_core::SecretString,
}
...
pub(in crate::features) struct TransferSettingsPresentation {
    pub(in crate::features) duplicate_policy: zzclawterm_transport::SftpDuplicatePolicy,
}

impl TransferSettingsPresentation {
    pub(in crate::features) fn duplicate_policy(
        &self,
    ) -> zzclawterm_transport::SftpDuplicatePolicy {
...
impl FontSelectOptionCache {
    fn empty() -> Self {
        Self {
            state: None,
            generation: None,
            options: Arc::from(Vec::<ZzClawSelectOption>::new()),
    ...
    fn update(&mut self, state: FontCatalogLoadState, generation: u64, source: &[String]) {
        ...
        let options: Arc<[ZzClawSelectOption]> = source
            .iter()
            .map(|option| {
                ZzClawSelectOption::new(option.clone(), option.clone()).font_family(option.clone())
            })
            .collect::<Vec<_>>()
            .into();
...
impl SettingsPanel {
    ...
    pub(in crate::features::pages::settings) fn font_select_option_catalog(
        &mut self,
        terminal: bool,
    ) -> (Arc<[ZzClawSelectOption]>, Arc<HashMap<String, String>>) {
        ...
        I: Into<SharedString>,
    {
        ...
        let mut select_element = ZzClawSelect::new(&select).appearance(true);
    ...
    pub(in crate::features::pages::settings) fn settings_select_field<I, L, S>(
        ...
        select: (Vec<ZzClawSelectOption>, S, bool),
        cx: &mut Context<Self>,
    ...
    ///
    /// The panel only ever looks inputs up: the entities are owned by `ZzClawTermApp`'s
    ...
    pub(in crate::features) fn existing_text_input(
        &self,
        id: impl AsRef<str>,
    ) -> Option<Entity<zzclawterm_ui::ZzClawInputState>> {
    ...
    pub(in crate::features) fn existing_text_input_box(
        ...
        match self.existing_text_input(&id) {
            Some(field) => {
                let shell = ZzClawInputShell::new(id, &field);
                if multi_line {
    ...
    /// `ZzClawTermApp::number_input` returns early for an id it already has.
    ...
    fn create_select_entity(
        ...
        let select =
            cx.new(|cx| ZzClawSelectState::new(cx, options, selected_value).disabled(disabled));
    ...
    fn create_select_entity_shared(
        ...
        let select = cx.new(|cx| {
            ZzClawSelectState::new_shared(cx, options, selected_value).disabled(disabled)
        });
        self.register_select_entity(id, select, cx)
    }
        ...
        let subscription =
            cx.subscribe(
                ...
                move |panel: &mut SettingsPanel, _, event, cx| match event {
                    zzclawterm_ui::ZzClawSelectEvent::Changed(value) => {
...
impl SettingsPanel {
    ...
    fn settings_nav_groups(&self, snapshot: &SettingsSnapshot) -> Vec<ZzClawSettingsNavGroup> {
        let palette = snapshot.chrome.palette;
        vec![
...
fn settings_nav_item(tab: SettingsTab) -> ZzClawSettingsNavItem {
    ZzClawSettingsNavItem::new(
...
mod tests {
    ...
    use crate::features::{FontCatalogLoadState, ZzClawTermApp};
    ...
    fn main_panel(app: &Entity<ZzClawTermApp>, cx: &mut gpui::App) -> Entity<SettingsPanel> {
    ...
    fn panel_lease_callback_does_not_double_lease() {
        let test_dir = TestConfigDir::new("zzclawterm-settings-panel");
    ...
    fn activate(app: &Entity<ZzClawTermApp>, vcx: &mut VisualTestContext, tab: SettingsTab) {
        vcx.update(|_, cx| {
    ...
    fn from_panel(
        ...
        f: impl FnOnce(&mut ZzClawTermApp, &mut gpui::Window, &mut gpui::Context<ZzClawTermApp>),
    ) {
        vcx.update(|window, cx| {
            app.update(cx, |app, cx| {
                f(app, window, cx);
    ...
    fn drawn_text_inputs(app: &Entity<ZzClawTermApp>, vcx: &mut VisualTestContext, ids: &[&str]) {
        vcx.update(|_, cx| {

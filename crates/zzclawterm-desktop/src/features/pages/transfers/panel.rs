...
pub(in crate::features) struct TransferBrowserPresentation {
    ...
    pub search_field: Option<Entity<ZzClawInputState>>,
    pub path_field: Option<Entity<ZzClawInputState>>,
    /// Built where the rename dialog opens, so the virtualised row builder can show
    /// the field without being able to create one.
    pub rename_field: Option<Entity<ZzClawInputState>>,
...
impl TransferPanel {
    ...
    pub(in crate::features::pages::transfers) fn app_handle(&self) -> WeakEntity<ZzClawTermApp> {
        self.app.clone()
    }
...
impl Render for TransferPanel {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // Zero `ZzClawTermApp` access, diagnostics included: GPUI records every entity
...
mod tests {
    ...
    fn duplicate_prompts_open_globally_without_panel_interaction_and_drain_in_order() {
        ...
        use zzclawterm_ui::{ZzClawDialogWindowExt as _, zzclaw_root};

        let test_dir = TestConfigDir::new("zzclawterm-transfer-duplicate");
    ...
    fn an_unrelated_app_notify_does_not_repaint_the_panel() {
        let test_dir = TestConfigDir::new("zzclawterm-transfer-panel");
        let mut cx = TestAppContext::single();

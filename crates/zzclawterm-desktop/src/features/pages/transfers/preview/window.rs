...
use zzclawterm_ui::{
    ZzClawScrollable, ZzClawTooltip, ZzClawWindowHandle, activate_child_window, zzclaw_root,
};
...
impl RemoteFilePreviewWindow {
    fn new(app: Entity<ZzClawTermApp>, chrome: ChildWindowChrome, cx: &mut Context<Self>) -> Self {
    ...
    fn toolbar_button(
        ...
        div()
            ...
            .tooltip({
                ...
                move |window, cx| ZzClawTooltip::new(tooltip.clone()).build(window, cx)
            })
...
fn preview_category_label(tab: &TransferPreviewState) -> String {
    use zzclawterm_core::PreviewCategory;
    match tab.category {
...
fn apply_preview_zoom(app: &mut ZzClawTermApp, factor: f32, is_pdf: bool) -> bool {
    if is_pdf {
        app.transfer
    ...
    pub(in crate::features) fn open_remote_file_preview_window(&mut self, cx: &mut Context<Self>) {
        if let Some(handle) = self.transfer.preview_window() {
            activate_child_window(
                &cx.entity(),
                handle,
                |app: &mut ZzClawTermApp| Some(app.transfer.preview_window_slot()),
                cx,
            );
            return;
        }
...
fn open_remote_file_preview_window_now_from_app(app: Entity<ZzClawTermApp>, cx: &mut App) {

...
use zzclawterm_ui::{ZzClawDropdownMenu, ZzClawMenuItem};
...
pub(super) fn network_item_overflow_menu(
    ...
    let mut items = vec![
        ZzClawMenuItem::action(edit_label)
            .icon("icons/net/edit.svg")
            .on_click(on_edit),
    ];
    if can_move {
        items.push(
            ZzClawMenuItem::action(move_label)
    ...
    items.extend([
        ...
        ZzClawMenuItem::action(delete_label)
            .icon("icons/net/delete.svg")
            .danger()
            .on_click(on_delete),
    ]);

    ZzClawDropdownMenu::new(id)

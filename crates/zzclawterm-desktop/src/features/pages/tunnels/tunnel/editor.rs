...
use crate::features::{ZzClawTermApp, selects::NO_SELECTION_VALUE, text_inputs::TextInputSetup};
...
pub(in crate::features::pages::tunnels) fn network_tunnel_editor_content(
    ...
    let type_options = vec![
        ZzClawSelectOption::new("local", t!("network.localTunnel")),
        ZzClawSelectOption::new("remote", t!("network.remoteTunnel")),
        ZzClawSelectOption::new("dynamic", t!("network.dynamicTunnel")),
    ];
    ...
    .to_string();
    let mut group_options = vec![ZzClawSelectOption::new(
        NO_SELECTION_VALUE,
        t!("network.ungrouped"),
    )];
    group_options.extend(
        app.tunnel_state
            .tunnel_groups()
            .iter()
            .map(|group| ZzClawSelectOption::new(group.id.clone(), group.name.clone())),
    );
    ...
    let mut connection_options = vec![ZzClawSelectOption::new(
        NO_SELECTION_VALUE,
    ...
    connection_options.extend(
        app.connection_state
            ...
            .map(|connection| {
                ZzClawSelectOption::new(connection.id.clone(), connection.name.clone())
            }),
    );
    ...
    if matches!(
        ...
        return div()
            ...
            .child(app.number_input_box(
                ...
                &value,
                ZzClawNumberInputOptions::default().range(1.0, 65535.0),
                cx,
            ))
            .into_any_element();
    }

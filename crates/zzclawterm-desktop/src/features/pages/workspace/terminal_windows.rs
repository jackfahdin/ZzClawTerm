...
use zzclawterm_transport::{SessionInfo, SessionKind};
        ...
        match node {
            TerminalWindowNode::Leaf {
                ...
                let trigger = div()
                    ...
                    .tooltip(|window, cx| {
                        zzclawterm_ui::ZzClawTooltip::new(rust_i18n::t!("terminal.newSession"))
                            .build(window, cx)
                    });
                let popover_anchor = menu_anchor.clone();
                let popover = ZzClawPopover::new(
...
fn multi_leaf_session_kind_icon(kind: zzclawterm_transport::SessionKind) -> &'static str {
    match kind {
        zzclawterm_transport::SessionKind::Ssh => "icons/conn/server.svg",
        zzclawterm_transport::SessionKind::Telnet | zzclawterm_transport::SessionKind::RawTcp => {
            "icons/conn/telnet.svg"
        }
        zzclawterm_transport::SessionKind::Serial => "icons/conn/serial.svg",
        zzclawterm_transport::SessionKind::LocalPty => "icons/conn/terminal.svg",
        zzclawterm_transport::SessionKind::Rdp => "icons/conn/server.svg",
        zzclawterm_transport::SessionKind::Vnc => "icons/conn/server.svg",
    }
}

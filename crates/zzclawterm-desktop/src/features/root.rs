...
///
/// Computed from `ZzClawTermApp` directly. This used to be read back from a
...
impl ZzClawTermApp {
    ...
    pub(in crate::features) fn notify_operation(
        &self,
        key: &'static str,
        kind: zzclawterm_ui::notification::ZzClawNotificationKind,
        message: String,
        cx: &mut Context<Self>,
    ) {
        let target = cx
        ...
        if let Some(target) = target {
            cx.defer(move |cx| {
                let _ = target.update(cx, |_, window, cx| {
                    use zzclawterm_ui::notification::ZzClawNotificationWindowExt as _;
    ...
    pub(crate) fn start_after_window_open(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self.maybe_autostart_x11_server();
    ...
    fn root_chrome(&mut self, window: &mut Window, cx: &mut Context<Self>) -> Stateful<Div> {
        ...
        let root = div()
            .id(SharedString::from("zzclawterm-root"))
...
mod tests {
    ...
    fn modal_predicate_app(cx: &mut TestAppContext) -> gpui::Entity<super::ZzClawTermApp> {
        let root = std::env::temp_dir().join(format!(
            "zzclawterm-modal-predicate-{}-{}",
            std::process::id(),
            uuid()
        ));
        ...
        cx.new(|cx| super::ZzClawTermApp::new(runtime, stores, cx))
    }
    ...
    fn root_render_benchmark_dir() -> PathBuf {
        std::env::temp_dir().join(format!(
            "zzclawterm-root-render-benchmark-{}-{}",
            std::process::id(),
            uuid()
        ))
    }
    ...
    struct RootRenderBenchmarkFixture {
        app: gpui::Entity<super::ZzClawTermApp>,
    }
    ...
    fn run_root_render_benchmark(cx: &mut TestAppContext) {
        ...
        let app = cx.new(|cx| super::ZzClawTermApp::new(runtime, stores, cx));
    ...
    fn root_render_hundred_sessions_eight_terminal_leaves_benchmark() {
        std::thread::Builder::new()
            .name("zzclawterm-root-render-benchmark".to_string())

...
use zzclawterm_core::DecryptedOtpEntry;

use crate::models::event_wake::{ANY_INTEREST, EventWake};
use zzclawterm_store::{KnownHostCheck, StoreBlockingClient, StoreDomain};
...
impl SshHostKeyVerifier for NativeHostKeyVerifier {
    fn for_attempt(
        &self,
        attempt: zzclawterm_transport::connection_attempt::ConnectionAttempt,
...
impl NativeOtpProvider {
    ...
    fn generate_totp_code(&self, entry: &DecryptedOtpEntry, now: u64) -> Result<TotpCode, String> {
        ...
        let totp = zzclawterm_otp::Totp::new(
            algorithm,
            entry.issuer.clone(),
    ...
    fn generate_hotp_code(&self, entry: &DecryptedOtpEntry) -> Result<String, String> {
        let (algorithm, secret, digits) = otp_material(entry)?;
        let mut hotp = zzclawterm_otp::Hotp::new(
            algorithm,
            entry.issuer.clone(),
            entry.username.clone(),
            digits,
...
fn otp_material(
...
) -> Result<(zzclawterm_otp::Algorithm, zzclawterm_otp::Secret, u8), String> {
    let secret = entry
        .secret
        .as_deref()
    ...
    let algorithm = match entry.algorithm.as_str() {
        "SHA256" => zzclawterm_otp::Algorithm::SHA256,
        "SHA512" => zzclawterm_otp::Algorithm::SHA512,
        _ => zzclawterm_otp::Algorithm::SHA1,
    };
    let secret = zzclawterm_otp::Secret::from_base32(secret)
...
fn agent_prompt_id() -> String {
    format!("agent-{}", zzclawterm_core::uuid())
}
...
mod prompt_state_debug_tests {
    ...
    fn agent_prompt_broker_delivers_retry_action() {
        ...
        let worker = {
            ...
            std::thread::Builder::new()
                .name("zzclawterm-agent-prompt-test".to_string())
    ...
    fn native_otp_provider_debug_redacts_used_codes() {
        let test_dir = TestConfigDir::new("zzclawterm-otp-debug-test");

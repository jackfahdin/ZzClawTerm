...
use gpui::Context;
use zzclawterm_core::{AiExecutionProfile, uuid};
...
use crate::features::{
    ZzClawTermApp, runtime_jobs::SessionStartResult, runtime_jobs::SessionStartSuccess,
...
impl ZzClawTermApp {
    ...
    pub(in crate::features) fn send_probe_command(&mut self, cx: &mut Context<Self>) {
        ...
        let command = if cfg!(target_os = "windows") {
            "echo zzclawterm-app-ready\r\n"
        } else {
            "printf 'zzclawterm-app-ready\\n'\n"
        };
        ...
        match event.result {
            Ok(mut success) => {
                ...
                if let Some(connection_id) = source_connection_id.as_deref() {
                    ...
                    if kind == zzclawterm_transport::SessionKind::Ssh {
    ...
    match info.kind {
        ...
        SessionKind::Telnet | SessionKind::RawTcp => {
            SessionLaunchConfig::Telnet(TelnetSessionConfig {
                ...
                enter_mode: zzclawterm_transport::TelnetEnterMode::default(),
                local_echo: false,
                local_line_edit: false,
                force_character_at_a_time: false,
                send_naws: false,
                send_sga: false,
                auto_login: zzclawterm_transport::TelnetAutoLoginConfig::default(),
                encoding: "UTF-8".to_string(),
                cols: info.cols,
                rows: info.rows,
            })

...
impl SessionFeatureState {
    ...
    /// Taken once by `ZzClawTermApp::start_runtime_data_plane_drain`.
...
pub(super) struct SessionStartFeatureState {
    ...
    /// Taken once by `ZzClawTermApp::start_session_start_event_drain`, which owns
...
impl SessionStartFeatureState {
    ...
    pub(in crate::features) fn attempt(
        &self,
        request_id: &str,
    ) -> zzclawterm_transport::connection_attempt::ConnectionAttempt {
        self.pending
            .get(request_id)

...
//! in `zzclawterm-core`; the maps below hold values the user has explicitly
...
///
/// These live on the state rather than on `ZzClawTermApp` so closing an editor or
    ...
    fn opening_an_editor_replaces_the_previous_editor() {
        ...
        security.open_key_editor(
            SecurityKeyEditorState {
                ...
                key_data: zzclawterm_core::SecretString::default(),
                cert_file_path: String::new(),
                cert_data: zzclawterm_core::SecretString::default(),
                passphrase: zzclawterm_core::SecretString::default(),

...
use crate::features::{
    ZzClawTermApp, session::credential_prompt_target, session::keyboard_interactive_prompt_target,
...
impl ZzClawTermApp {
    ...
    pub(in crate::features) fn refresh_window_render_inputs(
        ...
        if let Some(handle) = window.window_handle().downcast::<ZzClawRoot>() {
...
mod tests {
    ...
    use zzclawterm_transport::{LocalSessionConfig, SessionEvent};
    ...
    impl RootNotifyObserver {
        fn new(app: gpui::Entity<ZzClawTermApp>, cx: &mut Context<Self>) -> Self {
    ...
    fn quiet_app_with_visible_session(cx: &mut TestAppContext) -> gpui::Entity<ZzClawTermApp> {

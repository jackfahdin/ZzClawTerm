...
use zzclawterm_core::fuzzy_search_items;
use zzclawterm_ui::ZzClawCommandState;

use crate::entities::{OverlayStore, QuickSwitchState};
use crate::features::{ZzClawTermApp, formatting::session_kind_label, formatting::short_id};
...
impl ZzClawTermApp {
    ...
    pub(in crate::features) fn quick_switch_command_state(
        &self,
        cx: &mut Context<Self>,
    ) -> Option<gpui::Entity<ZzClawCommandState>> {
    ...
    pub(in crate::features) fn open_quick_switch(
        ...
        let command_state = cx.new(|cx| ZzClawCommandState::new(window, cx));

...
//!
//! Shell rendering remains on `ZzClawTermApp` views. This state owns interaction
...
use zzclawterm_ui::{ChildWindowSlot, ZzClawAppMenuBar, ZzClawWindowHandle};
...
pub(in crate::features) struct ShellFeatureState {
    ...
    /// The window `ZzClawTermApp` itself renders in.
    ///
    ...
    main_window: Option<ZzClawWindowHandle>,
}
...
pub(super) struct ShellChromeState {
    pub(super) title_menu_bar: Option<Entity<ZzClawAppMenuBar>>,
...
impl ShellFeatureState {
    ...
    pub(in crate::features) fn set_title_menu_bar(&mut self, menu_bar: Entity<ZzClawAppMenuBar>) {
        self.chrome.title_menu_bar = Some(menu_bar);
    }

    pub(in crate::features) fn title_menu_bar(&self) -> Option<Entity<ZzClawAppMenuBar>> {
    ...
    pub(in crate::features) fn settings_window(&self) -> Option<ZzClawWindowHandle> {
    ...
    /// The window `ZzClawTermApp` renders in, once the first frame has run.
    pub(in crate::features) fn main_window(&self) -> Option<ZzClawWindowHandle> {
        self.main_window
    }

    /// Record the main window on the first frame that can see it.
    ///
    /// Only ever set once: `ZzClawTermApp` renders in exactly one window, and a
    /// child window's root is a different view type so it cannot land here.
    pub(in crate::features) fn remember_main_window(&mut self, handle: ZzClawWindowHandle) {

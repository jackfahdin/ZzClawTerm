...
//! protocol stay in `zzclawterm-terminal` and `zzclawterm-transport`.
...
pub(super) struct TerminalPasteReviewState {
    pub(super) editor: Option<Entity<ZzClawDocumentEditorState>>,
...
impl TerminalFeatureState {
    ...
    /// `ZzClawTermApp::open_terminal_search` performs, which needs a `Window`.
    #[cfg(test)]
    ...
    pub(in crate::features) fn paste_review_editor(
        &self,
    ) -> Option<Entity<ZzClawDocumentEditorState>> {

...
use zzclawterm_terminal::{TerminalLineId, TerminalScreen, TerminalSnapshot};
...
use crate::terminal::{
    TerminalGridSelection, TerminalKeywordHighlightSnapshot, TerminalKeywordHighlighter,
    TerminalLineDecorations, ZzClawTerminalElement, ZzClawTerminalLayoutCache,
...
fn terminal_keyword_rule_sets_equal(
    left: &Arc<Vec<zzclawterm_core::ResolvedKeywordHighlightRule>>,
    right: &Arc<Vec<zzclawterm_core::ResolvedKeywordHighlightRule>>,
) -> bool {
    Arc::ptr_eq(left, right) || left.as_ref() == right.as_ref()
}

fn empty_terminal_keyword_rules() -> Arc<Vec<zzclawterm_core::ResolvedKeywordHighlightRule>> {
    static RULES: OnceLock<Arc<Vec<zzclawterm_core::ResolvedKeywordHighlightRule>>> =
        OnceLock::new();
...
/// on `ZzClawTermApp` and is notified only for unread/effects/layout changes.
pub(in crate::features) struct TerminalSurface {
    session_id: String,
    /// Parent app for scroll/selection actions that still live on ZzClawTermApp.
    app: Option<WeakEntity<ZzClawTermApp>>,
    ...
    keyword_highlighter_rules: Option<Arc<Vec<zzclawterm_core::ResolvedKeywordHighlightRule>>>,
    ...
    show_cursor: bool,
    cursor_style: String,
    layout_cache: Arc<Mutex<ZzClawTerminalLayoutCache>>,
...
impl TerminalSurface {
    pub(in crate::features) fn new(session_id: impl Into<String>) -> Self {
        Self {
            ...
            show_cursor: false,
            cursor_style: "block".to_string(),
            layout_cache: Arc::new(Mutex::new(ZzClawTerminalLayoutCache::default())),
            show_line_numbers: false,
            show_timestamps: false,
            timestamp_format: zzclawterm_core::DEFAULT_TERMINAL_TIMESTAMP_FORMAT.to_string(),
            scroll_offset: 0,
    ...
    pub(in crate::features) fn set_app(&mut self, app: Entity<ZzClawTermApp>) {
    ...
    fn cached_keyword_highlighter_for_rules(
        &self,
        rules: &Arc<Vec<zzclawterm_core::ResolvedKeywordHighlightRule>>,
    ...
    fn flush_local_scroll_app_sync(&mut self, app: Entity<ZzClawTermApp>, cx: &mut Context<Self>) {
...
fn hidden_terminal_cursor_snapshot() -> zzclawterm_terminal::CursorSnapshot {
    zzclawterm_terminal::CursorSnapshot {
        row: usize::MAX,
        col: 0,
        shape: zzclawterm_terminal::CursorShape::Hidden,
        visible: false,
        blinking: false,
    }
}
...
impl Render for TerminalSurface {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let mut grid = ZzClawTerminalElement::new(
            snapshot.clone(),

...
use zzclawterm_terminal_gpui::{ZzClawTerminalLayoutCache, precompute_terminal_keyword_highlights};
...
fn keyword_highlight_request_key_tracks_snapshot_and_rules() {
    ...
    let snapshot = screen.viewport_snapshot(0);
    let rules = vec![zzclawterm_core::ResolvedKeywordHighlightRule {
        id: "alpha".to_string(),
...
fn keyword_highlight_deferred_task_coalesces_latest_snapshot_under_pressure() {
    ...
    let rules = Arc::new(vec![zzclawterm_core::ResolvedKeywordHighlightRule {
        id: "beta".to_string(),
        name: "Beta".to_string(),
...
fn cached_keyword_highlighter_reuses_equal_rule_sets() {
    let cached_rules = Arc::new(vec![zzclawterm_core::ResolvedKeywordHighlightRule {
        id: "alpha".to_string(),
        name: "Alpha".to_string(),
        patterns: vec!["alpha".to_string()],
        color: "#ff0000".to_string(),
...
fn scroll_without_target_snapshot_preserves_stale_paint_state() {
    ...
    let rows = snapshot.row_count();
    let mut surface = TerminalSurface::new("session");
    surface.keyword_rules = Arc::new(vec![zzclawterm_core::ResolvedKeywordHighlightRule {
        id: "test".to_string(),
        name: "test".to_string(),
...
fn protocol_scroll_wheel_defers_parent_update_until_surface_lease_is_released() {
    ...
    let root = std::env::temp_dir().join(format!(
        "zzclawterm-terminal-scroll-wheel-{}-{}",
        std::process::id(),
        uuid()
    ));

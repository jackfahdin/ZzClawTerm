...
//!
//! Ordinary form, prompt and search inputs use [`ZzClawInputState`] entities
...
//!
//! The connection editor owns dedicated [`ZzClawInputState`] entities for fields
...
use zzclawterm_ui::{
    NYA_FORM_CONTROL_HEIGHT_PX, ZzClawInputEvent, ZzClawInputShell, ZzClawInputState,
...
pub(in crate::features) struct TextInputRegistry {
    fields: HashMap<SharedString, Entity<ZzClawInputState>>,
    number_fields: HashMap<SharedString, Entity<ZzClawNumberInputState>>,
...
impl ZzClawTermApp {
    ...
    pub(in crate::features) fn text_input_fields_snapshot(
        &self,
    ) -> HashMap<SharedString, Entity<ZzClawInputState>> {
        self.text_inputs.fields.clone()
    }

    pub(in crate::features) fn number_input_fields_snapshot(
        &self,
    ) -> HashMap<SharedString, Entity<ZzClawNumberInputState>> {
    ...
    fn text_input_with_language(
        ...
        let entity = cx.new(|cx| {
            let input = ZzClawInputState::new(cx, seed.to_string()).placeholder(setup.placeholder);
    ...
    pub(in crate::features) fn search_input_box<I: Into<SharedString>>(
        ...
        let id = id.into();
        let field = self.text_input(id.clone(), seed, setup, cx);
        ZzClawSearchInput::new(id, &field)
    }
    ...
    pub(in crate::features) fn number_input(
        ...
        id: impl Into<SharedString>,
        seed: &str,
        setup: ZzClawNumberInputOptions,
        cx: &mut Context<Self>,
    ) -> Entity<ZzClawNumberInputState> {
        let id = id.into();
        if let Some(field) = self.text_inputs.number_fields.get(&id) {
            return field.clone();
        }

        let entity = cx.new(|cx| ZzClawNumberInputState::new(cx, seed.to_string(), setup));
        let subscription_id = id.clone();
        let subscription =
            cx.subscribe(
                &entity,
...
pub(in crate::features) fn number_input_box_from_state(
    id: SharedString,
    palette: crate::theme::ThemePalette,
    field: Entity<ZzClawNumberInputState>,
) -> impl IntoElement {
    div()
        .id(id)

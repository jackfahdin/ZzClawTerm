...
use zzclawterm_core::document_edit::{
...
pub(in crate::features) struct RemoteTextEditor {
    ...
    projection: zzclawterm_core::document_edit::FoldProjection,
    ...
    search_input: Entity<ZzClawInputState>,
    replace_input: Entity<ZzClawInputState>,
    ...
    fn on_key_down(&mut self, event: &KeyDownEvent, window: &mut Window, cx: &mut Context<Self>) {
            ...
            && matches!(key, "backspace" | "delete")
        {
            let edits = self
                ...
                .collect();
            let ranges =
                zzclawterm_core::document_edit::normalize_selections(&self.document.content, edits);
            let edits = ranges
                .into_iter()
                .map(|range| zzclawterm_core::document_edit::TextEdit {
                    range,
                    text: String::new(),
                })
                .collect();
...
impl Render for RemoteTextEditor {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let selection = self.selected_range();
        self.projection = zzclawterm_core::document_edit::FoldProjection::new(
            &self.document.content,
            self.folds.clone(),
        );

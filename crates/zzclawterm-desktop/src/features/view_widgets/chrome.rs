...
pub(in crate::features) fn logo_mark(palette: ThemePalette) -> impl IntoElement {
    div()
        ...
        .items_center()
        .justify_center()
        .child(super::icons::zzclawterm_app_icon(palette, 22.))
}
...
/// matches the window-wide contract documented in `zzclawterm_ui::root`: above the
...
pub(in crate::features) fn dialog_action_button(
    ...
    ZzClawButton::new(id.into(), label)
        .variant(variant)
        .small()
        .compact()
        .on_click(on_click)

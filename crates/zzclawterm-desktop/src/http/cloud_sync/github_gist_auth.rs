...
const GITHUB_GIST_CLIENT_ID: Option<&str> = option_env!("ZZCLAWTERM_GITHUB_GIST_CLIENT_ID");
...
struct AccessTokenResponse {
    #[serde(default)]
    access_token: Option<zzclawterm_core::SecretString>,
    #[serde(default)]
    scope: Option<String>,
    #[serde(default)]
    error: Option<String>,
...
fn resolve_github_gist_id(
    ...
    let payload: GithubGistResponse = decode_json(
        ...
        .json(&serde_json::json!({
            "description": "ZzClawTerm encrypted cloud sync storage",
            "public": false,
            "files": {
                "zzclawterm-readme.txt": {
                    "content": "This private gist stores encrypted ZzClawTerm cloud sync objects."
                }
            }
        }))
        .send()

...
/// `zh` and then to English because no `zh` catalog exists. ZzClawTerm has persisted
...
/// The locales ZzClawTerm ships a catalog for, in a stable, authoritative order.
///
...
pub(crate) fn language_options() -> Vec<zzclawterm_ui::ZzClawSelectOption> {
    available_locales()
        .into_iter()
        .map(|locale| {
            let label = locale_display_name(&locale);
            zzclawterm_ui::ZzClawSelectOption::new(locale.into_owned(), label)
        })
        .collect()
}
...
mod tests {
    ...
    ///
    /// `quickCommands.commandPlaceholder` documents ZzClawTerm's own quick-command

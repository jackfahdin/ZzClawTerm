...
use crate::{
    ...
    terminal::{
        TerminalBufferMatch, TerminalLineDecorations, ZzClawTerminalLayoutCache,
...
impl TerminalProtocolState {
    ...
    pub(crate) fn alternate_scroll_payload(self, delta_lines: i32) -> Option<Vec<u8>> {
        ...
        let unit =
            zzclawterm_terminal::alternate_scroll_key_bytes(up, self.application_cursor_keys);
...
pub(crate) struct TerminalRenderCache {
    pub(crate) layout_cache: Arc<Mutex<ZzClawTerminalLayoutCache>>,
...
impl Default for TerminalFramePipeline {
    fn default() -> Self {
        let recording_writer =
            super::RecordingWritePipeline::spawn(zzclawterm_transport::DEFAULT_MEMORY_LIMIT_BYTES)
                .writer();
...
impl SelectedOccurrenceSearchJob {
    ...
    fn process_chunk(
        &mut self,
        session: &TerminalFrameSession,
    ) -> Result<bool, zzclawterm_terminal::TerminalSearchError> {

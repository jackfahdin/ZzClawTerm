...
fn terminal_frame_visible_text_event_keeps_only_tail_for_ui() {
    let mut session = TerminalFrameSession::new("UTF-8", 1000);
    let mut recording_pipeline = super::super::RecordingWritePipeline::spawn(
        zzclawterm_transport::DEFAULT_MEMORY_LIMIT_BYTES,
    );
...
fn terminal_frame_worker_batches_output_burst_into_single_frame() {
    ...
    let recording_pipeline = super::super::RecordingWritePipeline::spawn(
        zzclawterm_transport::DEFAULT_MEMORY_LIMIT_BYTES,
    );
    let mut pending = VecDeque::new();
...
fn selected_occurrence_search_deduplicates_soft_wrap_matches_across_chunks() {
    ...
    let expected = session
        .screen
        .search_grid(&zzclawterm_terminal::TerminalSearchQuery {
            pattern: key.query.clone(),
            regex: false,
            case_sensitive: true,
            whole_word: false,
            direction: zzclawterm_terminal::TerminalSearchDirection::Forward,
            limit: key.limit,
        })
        .unwrap();

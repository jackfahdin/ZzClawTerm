...
/// the late worker then recreates `config/zzclawterm.redb` after cleanup.
...
fn test_config_dir_removes_its_tree_on_drop() {
    let path = {
        let dir = TestConfigDir::new("zzclawterm-test-cleanup");
...
fn synchronized_test_store_cannot_recreate_its_tree_after_cleanup() {
    let path = {
        let dir = TestConfigDir::new("zzclawterm-test-store-cleanup");
        let path = dir.path().to_path_buf();
        let store = blocking_test_store(dir.path());
        assert!(path.join("config/zzclawterm.redb").is_file());
        drop(store);
        path
    };

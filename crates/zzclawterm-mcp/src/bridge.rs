...
async fn connection_rpc(
    ...
    match tokio::time::timeout(timeout, connection_rpc_inner(connection, method, params)).await {
        ...
        Err(_) => Err(ConnectionRpcError::Disconnected(bridge_error(
            "bridge_timeout",
            "ZzClawTerm MCP bridge request timed out.",
        ))),
    }
}
    ...
    let line = read_rpc_line(&mut connection.reader)
        ...
        .ok_or_else(|| {
            std::io::Error::new(
                ...
                "ZzClawTerm MCP bridge disconnected",
            )
        })?;
...
{
    let ephemeral = std::env::var("ZZCLAWTERM_MCP_EPHEMERAL").as_deref() == Ok("1");
    let host = std::env::var("ZZCLAWTERM_MCP_HOST").ok();
    let port = std::env::var("ZZCLAWTERM_MCP_PORT")
        .ok()
        .and_then(|value| value.parse().ok());
    let token = std::env::var("ZZCLAWTERM_MCP_TOKEN").ok();
    let generation = std::env::var("ZZCLAWTERM_MCP_GENERATION").ok();
    if let (Some(host), Some(port), Some(token), Some(generation)) = (host, port, token, generation)
    {
        if host != "127.0.0.1" {
            return Err("ZzClawTerm MCP bridge host must be 127.0.0.1".into());
        }
    ...
    if ephemeral {
        return Err("ZzClawTerm ephemeral MCP credential is incomplete or unavailable".into());
    }
    ...
    if document.version != PROTOCOL_VERSION || document.host != "127.0.0.1" {
        return Err("Unsupported or unsafe ZzClawTerm MCP discovery document".into());
    }
    Ok(BridgeEndpoint {
        host: document.host,
        port: document.port,
...
mod tests {
    ...
    use tokio_util::sync::CancellationToken;
    use zzclawterm_mcp_protocol::{RpcRequest, RpcResponse};
...
mod bridge_security_tests {
    ...
    use tokio_util::sync::CancellationToken;
    use zzclawterm_mcp_protocol::{MAX_RPC_LINE_BYTES, RpcRequest, RpcResponse};

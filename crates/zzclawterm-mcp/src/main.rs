...
impl ServerHandler for ZzClawTermMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            ...
            .with_instructions("Discover saved terminal connections, open sessions in ZzClawTerm, and operate scoped sessions. ZzClawTerm enforces session scope and approvals.")
    }
    ...
    async fn call_tool(
        ...
        if !self.tools.iter().any(|item| item.name == request.name) {
            return Err(McpError::invalid_params("Unknown ZzClawTerm tool", None));
        }
        ...
        let result = match self
            ...
            .await
        {
            Ok(value) => match validate_tool_result(&request.name, &value) {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let server = ZzClawTermMcp::new(endpoint)
...
mod tests {
    ...
    use zzclawterm_mcp_protocol::{MCP_TOOL_REGISTRY, RpcRequest, RpcResponse, tool};

    use super::{BridgeEndpoint, ZzClawTermMcp, build_tools};
    ...
    async fn mcp_initialize_lists_tools_and_calls_mock_bridge() {
            ...
            while let Some(line) = lines.next_line().await.unwrap() {
                ...
                let result = match request.method.as_str() {
                    ...
                    "capability.execute" => match request.params["tool"].as_str().unwrap() {
                        ...
                        tool::SESSION_OPEN => {
                            assert_eq!(request.params["arguments"]["connectionId"], "connection-1");
                            json!({
                                "sessionId": "session-2",
        ...
        let server_task = tokio::spawn(async move {
            let service = ZzClawTermMcp::new(BridgeEndpoint::for_test(port))

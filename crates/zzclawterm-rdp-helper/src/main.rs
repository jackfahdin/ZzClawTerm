...
async fn main() {
    ...
    match std::env::var("ZZCLAWTERM_RDP_HELPER_TEST_MODE").as_deref() {
    ...
    while let Some(message) = control_rx.recv().await {
        ...
        match message {
            ...
            RdpControlMessage::Connect { session_id, config } => {
                ...
                client_task = Some(
                    thread::Builder::new()
                        .name("zzclawterm-ironrdp".to_string())
                        .spawn(move || {
...
fn spawn_stdout_writer(
    ...
    thread::Builder::new()
        .name("zzclawterm-rdp-stdout".to_string())
        .spawn(move || {
...
fn spawn_stdin_reader(
    ...
    thread::Builder::new()
        .name("zzclawterm-rdp-stdin".to_string())
        .spawn(move || {
...
fn build_config(
    ...
    let clipboard_enabled = !matches!(
        ...
        zzclawterm_remote_desktop::RdpClipboardMode::Disabled
    );
...
async fn forward_output(
    ...
    loop {
        ...
        let result: Result<(), ()> = match event {
            ...
            // ZzClawTerm does not drive: connection/logon milestones (the helper derives
...
mod tests {
    ...
    fn first_complete_desktop_update_resets_and_converts_rgb_to_bgra() {
        ...
        let zzclawterm_remote_desktop::RdpFrameEvent::Bitmap {
            epoch,
            full,
            x,
            y,

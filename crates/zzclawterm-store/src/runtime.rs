...
impl zzclawterm_core::CloudLocalStore for StoreBlockingClient {
    fn encode_sync_snapshot(
        &self,
        snapshot: &zzclawterm_core::RawPortableSnapshot,
        master_password: &str,
    ) -> Result<Vec<u8>, zzclawterm_core::CloudSyncError> {
    ...
    fn decode_sync_pointer(
        &self,
        bytes: &[u8],
    ) -> Result<zzclawterm_core::RemoteSyncPointer, zzclawterm_core::CloudSyncError> {
    ...
    fn build_sync_snapshot(
        &self,
        options: &zzclawterm_core::LocalCloudSyncOptions,
    ) -> Result<zzclawterm_core::RawPortableSnapshot, zzclawterm_core::CloudSyncError> {
    ...
    fn persist_cloud_sync_state(
        &self,
        state: &zzclawterm_core::CloudSyncState,
    ) -> Result<(), zzclawterm_core::CloudSyncError> {
...
) -> Result<T, zzclawterm_core::CloudSyncError> {
    let event =
        response.map_err(|error| zzclawterm_core::CloudSyncError::LocalStore(error.to_string()))?;
    event.outcome.map_err(|error| {
        zzclawterm_core::CloudSyncError::LocalStore(format!(
            "{}: {}",
            error.category(),
            error.user_message()
        ))
...
pub struct BootstrapSnapshot {
    ...
    pub custom_icons: Vec<zzclawterm_core::models::sessions::ConnectionCustomIcon>,

...
struct ConnectionPasswordRecord {
    ...
    password: zzclawterm_core::SecretString,
    created_at_ms: u64,
    updated_at_ms: u64,
}
...
impl ConnectionStore {
    ...
    pub fn load_sessions(&self) -> Result<SessionsConfig, StorageError> {
        ...
        for connection in &mut connections {
            ...
            let Some(icon) =
                zzclawterm_core::models::sessions::ConnectionCustomIcon::from_legacy_data_url(
                    value,
    ...
    pub fn list_connection_custom_icons(
        &self,
    ) -> Result<Vec<zzclawterm_core::models::sessions::ConnectionCustomIcon>, StorageError> {
        let mut icons: Vec<zzclawterm_core::models::sessions::ConnectionCustomIcon> =
    ...
    pub fn save_connection_custom_icon(
        &self,
        icon: &zzclawterm_core::models::sessions::ConnectionCustomIcon,
    ) -> Result<(), StorageError> {
...
fn decrypt_optional_secret(
    ...
    value: &Option<zzclawterm_core::SecretString>,
...
fn decrypt_legacy_plaintext_secret(
    crypto: &CredentialCrypto,
    master_key_token: Option<&str>,
    value: &zzclawterm_core::SecretString,
) -> Result<zzclawterm_core::SecretString, StorageError> {
    let value = value.expose_secret().trim();
    if value.is_empty() {
        return Ok(zzclawterm_core::SecretString::default());
    }
    let Some(master_key_token) = master_key_token else {
...
fn optional_secret_present(value: &Option<zzclawterm_core::SecretString>) -> bool {
...
fn validate_config_backup_file(
    ...
    let validation_dir = std::env::temp_dir().join(format!(
        "zzclawterm-config-backup-validate-{}-{}-{}",
        std::process::id(),
        current_time_ms(),
        uuid()
    ));

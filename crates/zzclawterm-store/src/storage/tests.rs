...
fn accounts_preserve_masking_legacy_references_and_portable_snapshot_contracts() {
    ...
    let mut snapshot = source
        .build_raw_portable_snapshot(
            zzclawterm_core::PortableSnapshotKind::Backup,
            "device",
            "test",
        )
        .unwrap();
...
fn exports_and_imports_native_redb_backup() {
    ...
    let backup_path = backup_dir.join("zzclawterm.redb");
...
fn app_settings_summary_reads_and_updates_host_key_policy() {
    ...
    let initial = serde_json::json!({
        ...
        "recording": {
            "base_path": "/tmp/zzclawterm-recordings",
            "auto_start": true,
            "default_mode": "raw",
    ...
    assert_eq!(summary.recording_path, "/tmp/zzclawterm-recordings");
    ...
    favorite_update.ui_file_explorer_view_mode = zzclawterm_core::TransferBrowserViewMode::List;
    ...
    recording_update.recording_path = "/var/log/zzclawterm".to_string();
    ...
    assert_eq!(saved_recording.recording_path, "/var/log/zzclawterm");
...
fn workspace_pane_layout_roundtrip() {
    ...
    let store = ConnectionStore::open(&dir).expect("store");
    let layout = zzclawterm_core::models::RestorableWorkspacePaneNode::Split {
        id: "split-1".to_string(),
        direction: "vertical".to_string(),
        ratio: 0.4,
        first: Box::new(zzclawterm_core::models::RestorableWorkspacePaneNode::Leaf {
            tab_index: 0,
        }),
        second: Box::new(zzclawterm_core::models::RestorableWorkspacePaneNode::Leaf {
            tab_index: 1,
        }),
    };
    store
...
fn open_tab_pane_root_expands_and_maps_layout() {
    let tab = zzclawterm_core::models::RestorableOpenTab {
        ...
        locked: true,
        active_pane_id: None,
        root: Some(zzclawterm_core::models::RestorablePaneNode::Split {
            id: "s".to_string(),
            direction: "horizontal".to_string(),
            ratio: 0.4,
            first: Box::new(zzclawterm_core::models::RestorablePaneNode::leaf_session(
                "a",
                "SSH",
                Some("c1".to_string()),
            )),
            second: Box::new(zzclawterm_core::models::RestorablePaneNode::leaf_session(
                "b", "Local", None,
            )),
        }),
    };
    ...
    match layout {
        zzclawterm_core::models::RestorableWorkspacePaneNode::Split {
            direction,
            ratio,
            first,
            second,
            ...
            assert_eq!(
                *first,
                zzclawterm_core::models::RestorableWorkspacePaneNode::Leaf { tab_index: 3 }
            );
            assert_eq!(
                *second,
                zzclawterm_core::models::RestorableWorkspacePaneNode::Leaf { tab_index: 4 }
            );
        }
...
fn open_tabs_roundtrip() {
    ...
    let store = ConnectionStore::open(&dir).expect("store");
    let tabs = vec![
        zzclawterm_core::models::RestorableOpenTab::with_leaf_root(
            "Local",
            "Local",
            None,
            Some("dev".to_string()),
        ...
        zzclawterm_core::models::RestorableOpenTab {
...
fn terminal_window_layout_roundtrip() {
    ...
    let store = ConnectionStore::open(&dir).expect("store");
    let layout = zzclawterm_core::models::RestorableTerminalWindowNode::Split {
        direction: "vertical".to_string(),
        ratio: 0.45,
        first: Box::new(
            zzclawterm_core::models::RestorableTerminalWindowNode::Leaf {
                tab_indexes: vec![0, 1],
...
fn legacy_workspace_fields_migrate_and_new_manifest_dual_writes_recent_workspace() {
    ...
    let store = ConnectionStore::open(&dir).expect("store");
    let legacy_tab =
        zzclawterm_core::RestorableOpenTab::with_leaf_root("legacy", "Local", None, None, None);
    store
    ...
    assert_eq!(
        ...
        zzclawterm_core::WorkspaceId::legacy()
    );
    assert_eq!(migrated.workspaces[0].sessions.open_tabs, vec![legacy_tab]);

    let first_id = migrated.workspaces[0].id;
    let second_id = zzclawterm_core::WorkspaceId::new();
    let mut second = zzclawterm_core::WorkspaceRestoreState::empty(second_id);
    let recent_tab = zzclawterm_core::RestorableOpenTab::with_leaf_root(
        "recent",
        "SSH",
    ...
    let manifest = zzclawterm_core::WorkspaceRestoreManifest {
        version: zzclawterm_core::WORKSPACE_RESTORE_MANIFEST_VERSION,
...
fn device_window_manifest_roundtrips_and_projects_recent_window_to_legacy_key() {
    ...
    let store = ConnectionStore::open(&dir).expect("store");
    let first_id = zzclawterm_core::WorkspaceId::new();
    let second_id = zzclawterm_core::WorkspaceId::new();
    ...
    let manifest = zzclawterm_core::DeviceWindowManifest {
        version: zzclawterm_core::DEVICE_WINDOW_MANIFEST_VERSION,
        windows: vec![
            zzclawterm_core::DeviceWindowState {
...
fn empty_existing_portable_key_is_rejected() {
    ...
    assert!(matches!(
        ...
        Err(StorageError::Crypto(
            zzclawterm_core::CredentialCryptoError::EmptyPortableKey(path)
        )) if path == key_path
    ));
    std::fs::remove_dir_all(dir).ok();
}
...
fn invalid_utf8_portable_key_is_not_replaced() {
    ...
    assert!(matches!(
        result,
        Err(StorageError::Crypto(
            zzclawterm_core::CredentialCryptoError::ReadPortableKey { path, .. }
        )) if path == key_path
    ));
...
fn translation_settings_read_legacy_plaintext_and_encrypt_on_save() {
    ...
    update.deepl_api_key = zzclawterm_core::MASKED_SECRET_VALUE.to_string().into();
...
fn cloud_sync_settings_migrate_legacy_doc_on_save_without_losing_unknown_fields() {
    ...
    loaded.webdav.password = Some(zzclawterm_core::MASKED_SECRET_VALUE.into());
...
fn cloud_sync_settings_encrypt_and_merge_masked_provider_secrets() {
    ...
    masked_update.webdav.password = Some(zzclawterm_core::MASKED_SECRET_VALUE.to_string().into());
...
fn ai_settings_encrypt_and_merge_masked_provider_secrets() {
    ...
    assert_eq!(saved.default_mode, zzclawterm_core::AiMode::Agent);
    assert_eq!(
...
/// from a backup.
fn synced_agent_endpoint() -> zzclawterm_core::SshAgentEndpoint {
    #[cfg(windows)]
    return zzclawterm_core::SshAgentEndpoint::WindowsOpenSsh;
    #[cfg(not(windows))]
    return zzclawterm_core::SshAgentEndpoint::UnixSocket {
        path: "/run/user/1000/agent.sock".to_string(),
    };
}

/// Endpoint standing in for a device-local setting that must never be synced away.
fn device_local_agent_endpoint() -> zzclawterm_core::SshAgentEndpoint {
    #[cfg(windows)]
    return zzclawterm_core::SshAgentEndpoint::Pageant;
    #[cfg(not(windows))]
    return zzclawterm_core::SshAgentEndpoint::UnixSocket {
...
fn sync_snapshot_strips_device_local_ssh_agent_settings() {
    ...
    *agent_forwarding_config = Some(zzclawterm_core::SshAgentForwardingConfig {
        enabled: true,
        sources: zzclawterm_core::SshAgentForwardingSources {
            external_agent: true,
            external_agent_endpoints: vec![synced_agent_endpoint()],
            stored_keys: false,
        },
        policy: zzclawterm_core::SshAgentForwardingPolicy::All,
    });
    ...
    assert_eq!(
        auth_agent_endpoint,
        &Some(zzclawterm_core::SshAgentEndpoint::Auto)
    );
    ...
    *agent_forwarding_config = Some(zzclawterm_core::SshAgentForwardingConfig::default());
    store
    ...
    assert_eq!(
        agent_forwarding_config,
        Some(zzclawterm_core::SshAgentForwardingConfig::default())
    );
...
pub(super) fn unique_temp_dir(name: &str) -> TestTempDir {
    TestTempDir::new(&format!("zzclawterm-core-{name}"))
}
...
fn home_wrapping_key() -> Key<Aes256Gcm> {
    ...
    hasher.update(b"zzclawterm-key-wrap-v1:");
...
fn portable_snapshot_preserves_notes_and_unknown_entities_after_apply() {
    ...
    zzclawterm_core::portable_snapshot::validate_raw_snapshot(&rebuilt)
...
fn portable_snapshot_rejects_tampered_unknown_entity_before_writing() {
    ...
    assert!(matches!(
        error,
        StorageError::PortableSnapshot(
            zzclawterm_core::PortableSnapshotError::EntitiesHashMismatch
        )
    ));
    assert_eq!(
        store
...
fn portable_snapshot_excludes_and_preserves_device_local_main_window_state() {
    ...
    let workspace_id = zzclawterm_core::WorkspaceId::new();
    source
        .save_device_window_manifest(&zzclawterm_core::DeviceWindowManifest {
            version: zzclawterm_core::DEVICE_WINDOW_MANIFEST_VERSION,
            windows: vec![zzclawterm_core::DeviceWindowState {
                workspace_id,

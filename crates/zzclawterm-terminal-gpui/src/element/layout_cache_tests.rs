...
fn edit_snapshot_row(
    ...
    row: usize,
    edit: impl FnOnce(&mut zzclawterm_terminal::TerminalSnapshotRow),
) {
...
fn layout_cache_stats_report_local_deltas_without_resetting_cache() {
    ...
    assert_eq!(
        ...
        super::ZzClawTerminalLayoutCacheStats {
            hits: 0,
            misses: 1,
            shape_calls: 1,
            shape_duration_us: 0,
...
fn styled_span_hash_tracks_style_changes() {
    let plain = vec![zzclawterm_terminal::StyledSpan {
        text: "same".to_string(),
        style: zzclawterm_terminal::CellStyle::default(),
    }];
    let bold_style = zzclawterm_terminal::CellStyle {
        bold: true,
        ..Default::default()
    };
    let bold = vec![zzclawterm_terminal::StyledSpan {
        text: "same".to_string(),
        style: bold_style,
    }];
...
fn row_layout_key_uses_revision_instead_of_hashing_display_text() {
    ...
    let element = ZzClawTerminalElement::new(
        Arc::new(snapshot),
        Arc::new(Vec::new()),
        Vec::new(),
        false,
...
fn precomputed_keyword_match_does_not_reuse_plain_pending_row() {
    ...
    edit_snapshot_row(&mut snapshot, 0, |row| {
        row.text = "ERROR".to_string();
        row.styled_spans = vec![zzclawterm_terminal::StyledSpan {
...
fn plain_row_fast_path_accepts_unstyled_rows() {
    let default_spans = [zzclawterm_terminal::StyledSpan {
...
fn plain_row_fast_path_rejects_enhanced_rows() {
    let styled = zzclawterm_terminal::CellStyle {
        bold: true,
        ..zzclawterm_terminal::CellStyle::default()
    };
    let styled_spans = [zzclawterm_terminal::StyledSpan {
        text: "bold".to_string(),
        style: styled,
    }];
    let keyword_rule = ResolvedKeywordHighlightRule {

...
pub struct ZzClawTerminalLayoutCacheStats {
    pub hits: u64,
    pub misses: u64,
    pub shape_calls: u64,
...
impl ZzClawTerminalLayoutCacheStats {
...
impl ZzClawTerminalLayoutCache {
    pub fn stats(&self) -> ZzClawTerminalLayoutCacheStats {
        ZzClawTerminalLayoutCacheStats {
            hits: self.hits,
            misses: self.misses,
...
pub struct ZzClawTerminalElement {
    ...
    layout_cache: Option<Arc<Mutex<ZzClawTerminalLayoutCache>>>,
    show_cursor: bool,
    cursor_style: String,
    cell_width: f32,
    cell_height: f32,
...
#[derive(Default)]
pub struct ZzClawTerminalPaintPlan {
...
fn terminal_ansi_spans_are_plain(ansi_spans: Option<&[zzclawterm_terminal::StyledSpan]>) -> bool {
    let Some(spans) = ansi_spans else {
        return true;
    };
    let default_style = zzclawterm_terminal::CellStyle::default();
    spans
        .iter()
...
fn terminal_link_underline_color(palette: zzclawterm_ui::ThemePalette) -> u32 {
    palette.text_muted
}
...
fn push_terminal_image_placeholder(
    ...
    above_text: bool,
    palette: zzclawterm_ui::ThemePalette,
    plan: &mut ZzClawTerminalPaintPlan,
) {
...
impl IntoElement for ZzClawTerminalElement {
    type Element = Self;
...
impl Element for ZzClawTerminalElement {
    type RequestLayoutState = ();
    type PrepaintState = ZzClawTerminalPaintPlan;
    ...
    fn prepaint(
        ...
        if visible_bounds.size.width <= px(0.) || visible_bounds.size.height <= px(0.) {
            return ZzClawTerminalPaintPlan::default();
        }
        ...
        let mut plan = ZzClawTerminalPaintPlan::default();
        let cell_w = self.cell_width.max(1.);
        let scale_factor = window.scale_factor();
        let cell_h = zzclawterm_core::terminal_snapped_cell_height(self.cell_height, scale_factor);
    ...
    fn paint(
        ...
        let started_at = Instant::now();
        let cell_height =
            zzclawterm_core::terminal_snapped_cell_height(self.cell_height, window.scale_factor());

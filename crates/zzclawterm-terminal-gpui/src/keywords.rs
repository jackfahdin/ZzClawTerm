...
impl TerminalKeywordHighlightSnapshot {
    ...
    pub fn matches_snapshot(
        ...
        _palette: zzclawterm_ui::ThemePalette,
    ) -> bool {
...
fn terminal_keyword_row_bytes(row: &zzclawterm_terminal::TerminalSnapshotRow) -> usize {
    row.cells
        .iter()
...
mod tests {
    ...
    fn set_snapshot_row_hyperlink(
        ...
        let rows = Arc::make_mut(&mut snapshot.row_data);
        let row = Arc::make_mut(&mut rows[row]);
        row.hyperlinks = vec![zzclawterm_terminal::HyperlinkSpan {
            start_col,
    ...
    fn test_render_cells(text: &str, cols: usize) -> Vec<zzclawterm_terminal::RenderCell> {
        let mut cells: Vec<zzclawterm_terminal::RenderCell> = Vec::new();
        for ch in text.chars() {
            ...
            cells.push(zzclawterm_terminal::RenderCell {
    ...
    fn keyword_highlight_benchmark() {
        let rules = zzclawterm_core::get_builtin_keyword_rules(true);
        let compiled = compile_keyword_rules(&rules);
        let cases = [
            (
                "no-match",

...
fn parser_ignores_shell_noise_and_reads_marked_values() {
    let output = b"welcome\n__ZZCLAWTERM_ENV_test__:START\n__ZZCLAWTERM_ENV_test__:VALUE:PATH:L3RtcA==\nhook-noise\nhook-after-value\n__ZZCLAWTERM_ENV_test__:VALUE_END:PATH\n__ZZCLAWTERM_ENV_test__:END\n";
    let variables = vec!["PATH".to_string()];
    let values = parse_base64_shell_output("__ZZCLAWTERM_ENV_test__", output, &variables).unwrap();
...
fn parser_rejects_invalid_utf8_without_accepting_corrupted_value() {
    let output = b"__ZZCLAWTERM_ENV_test__:START\n__ZZCLAWTERM_ENV_test__:VALUE:SECRET:_w==\n__ZZCLAWTERM_ENV_test__:VALUE_END:SECRET\n__ZZCLAWTERM_ENV_test__:END\n";
    let variables = vec!["SECRET".to_string()];

    let error =
        parse_base64_shell_output("__ZZCLAWTERM_ENV_test__", output, &variables).unwrap_err();
...
fn targeted_parser_distinguishes_empty_from_missing_values() {
    let marker = "__ZZCLAWTERM_ENV_test__";
    let output = concat!(
        "__ZZCLAWTERM_ENV_test__:START\n",
        "__ZZCLAWTERM_ENV_test__:VALUE:EMPTY:\n",
        "__ZZCLAWTERM_ENV_test__:VALUE_END:EMPTY\n",
        "__ZZCLAWTERM_ENV_test__:VALUE_END:MISSING\n",
        "__ZZCLAWTERM_ENV_test__:END\n",
    );
...
fn complete_parser_keeps_empty_values_and_ignores_noise() {
    let output = concat!(
        ...
        "__ZZCLAWTERM_ENV_test__:VALUE:EMPTY:\n",
        "__ZZCLAWTERM_ENV_test__:VALUE_END:EMPTY\n",
        "__ZZCLAWTERM_ENV_test__:VALUE:NOT-A-VARIABLE:not-base64!!!\n",
        "__ZZCLAWTERM_ENV_test__:VALUE_END:NOT-A-VARIABLE\n",
        "__ZZCLAWTERM_ENV_test__:END\n",
        "trailing noise\n",
    );

    let values =
        parse_complete_base64_shell_output("__ZZCLAWTERM_ENV_test__", output.as_bytes()).unwrap();
...
fn complete_parser_accepts_unicode_identifier_names() {
    ...
    let output = concat!(
        "__ZZCLAWTERM_ENV_test__:START\n",
        "__ZZCLAWTERM_ENV_test__:VALUE:变量:5Lit5paH\n",
        "__ZZCLAWTERM_ENV_test__:VALUE_END:变量\n",
        "__ZZCLAWTERM_ENV_test__:END\n",
    );
...
async fn batch_lookup_rejects_an_unbounded_request() {
    let variables = (0..=256).map(|index| format!("ZZCLAWTERM_BATCH_{index}"));
...
async fn missing_variable_is_cached_until_refresh() {
    let cache = ShellEnvironmentCache::new();
    let variable = format!("ZZCLAWTERM_TEST_MISSING_{}", uuid::Uuid::new_v4().simple());
...
fn powershell_shell_loader_uses_powershell_protocol() {
    ...
    assert!(script.contains("Write-Output '__ZZCLAWTERM_ENV_TEST__:START'"));
...
fn complete_cmd_loader_lists_all_environment_variables() {
    ...
    assert_eq!(
        script,
        "@echo off&chcp 65001 >nul&echo __ZZCLAWTERM_ENV_TEST__:START&set&echo __ZZCLAWTERM_ENV_TEST__:END"
    );
}
...
fn cmd_shell_output_parser_reads_only_requested_values() {
    ...
    let output = concat!(
        "__ZZCLAWTERM_ENV_TEST__:START \r\n",
        "__ZZCLAWTERM_ENV_TEST__:VARIABLE_START:PATH \r\n",
        "PATH=C:\\Windows\\System32;C:\\Tools=stable\r\n",
        "PATH_EXTRA=must-not-leak\r\n",
        "__ZZCLAWTERM_ENV_TEST__:VARIABLE_END:PATH \r\n",
        "__ZZCLAWTERM_ENV_TEST__:VARIABLE_START:MISSING \r\n",
        "Environment variable MISSING is not defined.\r\n",
        "__ZZCLAWTERM_ENV_TEST__:VARIABLE_END:MISSING \r\n",
        "__ZZCLAWTERM_ENV_TEST__:END \r\n",
    );
...
async fn cmd_shell_loader_caches_a_missing_variable() {
    let cache = ShellEnvironmentCache::with_shell_path_for_test(PathBuf::from("cmd.exe"));
    let variable = format!("ZZCLAWTERM_CMD_MISSING_{}", uuid::Uuid::new_v4().simple());
...
async fn shell_loader_reads_a_custom_exported_value() {
    ...
    fs::write(
            &shell,
            "#!/bin/sh\nexport ZZCLAWTERM_TEST_CUSTOM_AGENT=/tmp/zzclawterm-agent.sock\nexec /bin/sh \"$@\"\n",
        )
    ...
    let value = cache
        .refresh("ZZCLAWTERM_TEST_CUSTOM_AGENT")
        .await
...
async fn targeted_shell_lookup_preserves_empty_and_unset_values() {
    ...
    fs::write(
        &shell,
        "#!/bin/sh\nexport ZZCLAWTERM_TEST_TARGETED_EMPTY=\nunset HOME\nexec /bin/sh \"$@\"\n",
    )
    ...
    let variables = [
        "ZZCLAWTERM_TEST_TARGETED_EMPTY".to_string(),
        "HOME".to_string(),
    ];
    cache
    ...
    assert_eq!(
        cache
            .cached("ZZCLAWTERM_TEST_TARGETED_EMPTY")
            .unwrap()
            .as_ref()
            .map(EnvironmentValue::as_str),
        Some("")
...
async fn initialize_reads_the_complete_custom_shell_environment() {
    ...
    fs::write(
            &shell,
            "#!/bin/sh\nexport ZZCLAWTERM_TEST_COMPLETE_ALPHA=alpha\nexport ZZCLAWTERM_TEST_COMPLETE_EMPTY=\nexport ZZCLAWTERM_TEST_COMPLETE_MULTILINE='first\nZZCLAWTERM_TEST_COMPLETE_FAKE=not-a-variable'\nexec /bin/sh \"$@\"\n",
        )
    ...
    assert_eq!(
        snapshot
            .get("ZZCLAWTERM_TEST_COMPLETE_ALPHA")
            .as_ref()
            .map(EnvironmentValue::as_str),
        Some("alpha")
    );
    assert_eq!(
        snapshot
            .get("ZZCLAWTERM_TEST_COMPLETE_EMPTY")
            .as_ref()
            .map(EnvironmentValue::as_str),
        Some("")
    );
    assert_eq!(
        snapshot
            .get("ZZCLAWTERM_TEST_COMPLETE_MULTILINE")
            .as_ref()
            .map(EnvironmentValue::as_str),
        Some("first\nZZCLAWTERM_TEST_COMPLETE_FAKE=not-a-variable")
    );
...
async fn resolve_refreshes_the_complete_snapshot_once_for_a_new_variable() {
    ...
    fs::write(
            &shell,
            format!(
                "#!/bin/sh\nif [ -e '{state_path}' ]; then export ZZCLAWTERM_TEST_LATE_VALUE=ready; else : > '{state_path}'; fi\nexec /bin/sh \"$@\"\n"
            ),
        )
    ...
    assert!(
        cache
            .cached("ZZCLAWTERM_TEST_LATE_VALUE")
            .unwrap()
            .is_none()
    );

    let value = cache
        .resolve("ZZCLAWTERM_TEST_LATE_VALUE")
        .await
...
async fn cancelled_auto_refresh_does_not_leave_the_cache_stuck() {
    ...
    let variable = format!(
        "ZZCLAWTERM_TEST_CANCELLED_REFRESH_{}",
        uuid::Uuid::new_v4().simple()
    );
    let shell = root.join("shell");
    fs::write(
...
fn shell_reader_marker_is_not_retained_in_environment_snapshots() {
    let values = super::canonicalize_environment_values(std::collections::HashMap::from([
        (
            "ZZCLAWTERM_SHELL_ENV_READER".to_string(),
            EnvironmentValue::new("1".to_string()),
        ),
        (
            "ZZCLAWTERM_TEST_VISIBLE".to_string(),
            EnvironmentValue::new("ok".to_string()),
        ),
    ]));

    assert!(!values.contains_key("ZZCLAWTERM_SHELL_ENV_READER"));
    assert_eq!(
        values
            .get("ZZCLAWTERM_TEST_VISIBLE")
            .map(EnvironmentValue::as_str),
        Some("ok")
    );
}

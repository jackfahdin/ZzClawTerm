...
pub fn open_ssh_multiplex_handle(config: SshSessionConfig) -> anyhow::Result<SshMultiplexHandle> {
    ...
    let runtime = Arc::new(
        tokio::runtime::Builder::new_multi_thread()
            .enable_all()
            .thread_name("zzclawterm-ssh-multiplex")
            .build()
...
struct OpenSshShellSession {
    ...
    post_login: Option<zzclawterm_core::models::sessions::ConnectionPostLogin>,
    ...
    pub fn create_telnet_session_with_attempt(
        ...
        let session = TelnetTransport {
            ...
            backspace_as_bs: zzclawterm_core::terminal::connection_input::BackspaceMode::parse(
                ...
                == zzclawterm_core::terminal::connection_input::BackspaceMode::Backspace,
...
struct TelnetAutoLoginState {
    username: String,
    password: Option<zzclawterm_core::SecretString>,
    started_at: Instant,
    tail: String,
    sent_wake: bool,
    sent_username: bool,
...
fn ssh_client_config(config: &SshSessionConfig) -> anyhow::Result<Arc<russh::client::Config>> {
    let keepalive_interval = if config.keep_alive_interval_secs == 0
        || config.keep_alive_mode
            == zzclawterm_core::terminal::connection_input::KeepaliveMode::Disabled
    {
        None
    } else {
    ...
    Ok(Arc::new(russh::client::Config {
        ...
        keepalive_mode: match config.keep_alive_mode {
            zzclawterm_core::terminal::connection_input::KeepaliveMode::Strict => {

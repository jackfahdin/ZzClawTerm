...
impl RemoteCopyStaging {
    fn new() -> anyhow::Result<Self> {
        let path = std::env::temp_dir().join(format!("zzclawterm-copy-{}", uuid::Uuid::new_v4()));
        fs::create_dir(&path)?;
        Ok(Self(path))
    }
...
mod tests {
    ...
    fn local_copy_applies_skip_and_rename_duplicate_policies() {
        let root = zzclawterm_core::test_support::TestTempDir::new("zzclawterm-file-copy");

...
fn local_session_echoes_output() {
    ...
    manager
        .write(&info.id, b"printf zzclawterm-transport-ready\\n\n")
        .expect("write");
    ...
    assert!(
        String::from_utf8_lossy(&output).contains("zzclawterm-transport-ready"),
        "output was: {}",
...
fn local_command_receives_the_cached_shell_environment() {
    let snapshot = EnvironmentSnapshot::from_test(
        HashMap::from([(
            "ZZCLAWTERM_TEST_LOCAL_ENV".to_string(),
    ...
    assert_eq!(
        command
            .get_env("ZZCLAWTERM_TEST_LOCAL_ENV")
...
fn serial_invalid_port_reports_open_error() {
    ...
    let port_name = if cfg!(target_os = "windows") {
        r"\\.\ZzClawTermMissingPort".to_string()
    } else {
        "/dev/zzclawterm-missing-port".to_string()
    };
...
fn ssh_shell_integration_script_emits_osc7_and_ready_marker() {
    ...
    assert!(script.contains("ZzClawTermCommand"));
    assert!(script.contains("ZzClawTermReady:session-1"));
}
...
fn cwd_only_shell_scripts_omit_semantic_markers_and_bash_debug_trap() {
    ...
    for shell in [
        ...
        assert!(script.contains("ZzClawTermCommand"));
        assert!(script.contains("ZzClawTermReady:session-cwd-only"));
...
fn run_bash_injection_history_probe(
    ...
    writer
        .write_all(b"stty -echo\nprintf '__ZZCLAWTERM_PTY_READY__\\n'\n")
    ...
    loop {
        let ready = output
            .lock()
            .expect("bash history probe output lock")
            .windows(b"__ZZCLAWTERM_PTY_READY__".len())
            .any(|window| window == b"__ZZCLAWTERM_PTY_READY__");
        if ready {
            break;
        }
    ...
    let mut commands = String::from("history -c\nprintf '__ZZCLAWTERM_USER_BEFORE__\\n'\n");
    if !history_initially_enabled {
        commands.push_str("set +o history\n");
    }
    commands.push_str(&script);
    commands.push_str(
        "if [[ -o history ]]; then printf '__ZZCLAWTERM_HISTORY_STATE__:enabled\\n'; else printf '__ZZCLAWTERM_HISTORY_STATE__:disabled\\n'; fi\nprintf '__ZZCLAWTERM_HISTORY_BEGIN__\\n'\nHISTTIMEFORMAT= builtin history\nprintf '__ZZCLAWTERM_HISTORY_END__\\n'\nexit\n",
    );
...
fn bash_history_probe_section(output: &str) -> &str {
    let start_marker = "__ZZCLAWTERM_HISTORY_BEGIN__\r\n";
    let end_marker = "__ZZCLAWTERM_HISTORY_END__";
    let start = output
        .rfind(start_marker)
...
fn bash_inline_shell_integration_does_not_pollute_enabled_history() {
    for mode in [
        ...
        assert!(
            output.contains("__ZZCLAWTERM_HISTORY_STATE__:enabled"),
            "history was not restored for {mode:?}: {output:?}"
        );
        let history = bash_history_probe_section(&output);
        assert!(
            history.contains("__ZZCLAWTERM_USER_BEFORE__"),
            "pre-injection history was lost for {mode:?}: {history:?}"
        );
        for leaked in ["ZZCLAWTERM_INJ", "__zzclawterm_", "__nya_bp_"] {
            assert!(
...
fn bash_inline_shell_integration_preserves_disabled_history() {
    for mode in [
        ...
        assert!(
            output.contains("__ZZCLAWTERM_HISTORY_STATE__:disabled"),
...
fn run_uploaded_shell_history_probe(
    ...
    match shell {
        ...
        super::ShellKind::Fish => {
            ...
            command.env("fish_history", format!("zzclawterm_probe_{probe_id}"));
    ...
    let mut commands = match shell {
        ...
        super::ShellKind::Zsh => {
            "HISTSIZE=0\nHISTSIZE=1000\nprint -r -- '__ZZCLAWTERM_USER_BEFORE__'\n".to_string()
        }
        super::ShellKind::Fish => "printf '__ZZCLAWTERM_USER_BEFORE__\\n'\n".to_string(),
        super::ShellKind::PosixSh | super::ShellKind::Unknown => unreachable!(),
    };
    match initial_state {
    ...
    match shell {
        ...
        super::ShellKind::Zsh => commands.push_str(
            "fc -P 2>/dev/null\nprintf '__ZZCLAWTERM_HISTORY_STACK__:%s\\n' $?\nprintf '__ZZCLAWTERM_HISTORY_BEGIN__\\n'\nfc -l 1\nprintf '__ZZCLAWTERM_HISTORY_END__\\n'\nexit\n",
        ),
        super::ShellKind::Fish => commands.push_str(
            "if set -q fish_private_mode; printf '__ZZCLAWTERM_PRIVATE_STATE__:enabled\\n'; else; printf '__ZZCLAWTERM_PRIVATE_STATE__:disabled\\n'; end\nprintf '__ZZCLAWTERM_HISTORY_BEGIN__\\n'\nhistory\nprintf '__ZZCLAWTERM_HISTORY_END__\\n'\nexit\n",
        ),
        super::ShellKind::PosixSh | super::ShellKind::Unknown => unreachable!(),
    }
    write_pty_paced(&mut writer, &commands);
...
fn bash_uploaded_shell_integration_preserves_history_state_without_pollution() {
    for mode in [
        ...
        if let Some(output) = enabled {
            assert!(output.contains("__ZZCLAWTERM_HISTORY_STATE__:enabled\r\n"));
        } else {
            return;
        }
        ...
        assert!(disabled.contains("__ZZCLAWTERM_HISTORY_STATE__:disabled\r\n"));
    }
}
...
fn zsh_uploaded_shell_integration_restores_history_stack_without_pollution() {
    for mode in [
        ...
        assert!(
            output.contains("__ZZCLAWTERM_HISTORY_STACK__:1\r\n"),
...
fn fish_uploaded_shell_integration_preserves_private_mode_without_pollution() {
    for mode in [
        ...
        for (initial_state, expected) in [
            ...
            assert!(
                output.contains(&format!("__ZZCLAWTERM_PRIVATE_STATE__:{expected}\r\n")),
...
fn ssh_activation_and_persistent_scripts_match_rc_file_mode_contract() {
    ...
    assert!(activation.contains("ZzClawTermReady:session-1"));
    assert!(activation.contains("__zzclawterm_install_prompt full"));
    ...
    assert!(cwd_activation.contains("__zzclawterm_install_prompt cwd"));
    assert!(persistent.contains("__zzclawterm_install_prompt"));
    assert!(persistent.contains("ZzClawTermCommand:%s"));
    assert!(block.contains("# >>> zzclawterm shell integration >>>"));
...
fn ssh_osc_stripper_ignores_ready_marker_for_other_sessions() {
    ...
    let result = stripper.push(b"a\x1b]7777;ZzClawTermReady:session-2\x07b");
...
fn ssh_ready_marker_helpers_strip_current_and_legacy_markers() {
    ...
    let legacy = super::build_legacy_ssh_ready_marker("\x1b]7777;ZzClawTermReady:session-1\x07")
...
fn process_parser_detects_unsupported_marker() {
    assert!(is_process_list_unsupported(
        "warning\nZZCLAWTERM_PROCESS_UNSUPPORTED\n"
    ));
    assert!(!is_process_list_unsupported(
...
fn collect_output(manager: &SessionManager, session_id: &str, timeout: Duration) -> Vec<u8> {
    collect_output_until(manager, session_id, "zzclawterm-transport-ready", timeout)
}
...
fn ssh_keepalive_policy_reaches_the_client_configuration() {
    use zzclawterm_core::terminal::connection_input::KeepaliveMode;

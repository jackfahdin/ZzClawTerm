...
//! Each used to carry its own `Option<ZzClawWindowHandle>` plus an `open_pending`
...
use crate::root::ZzClawWindowHandle;

#[derive(Debug, Default, Clone, Copy, PartialEq, Eq)]
pub struct ChildWindowSlot {
    window: Option<ZzClawWindowHandle>,
    open_pending: bool,
}

impl ChildWindowSlot {
    pub fn handle(&self) -> Option<ZzClawWindowHandle> {
        self.window
    }
    ...
    pub fn finish_open(&mut self, handle: ZzClawWindowHandle) {
    ...
    pub fn clear_if(&mut self, handle: ZzClawWindowHandle) -> bool {
...
mod tests {
    ...
    fn clear_if_only_clears_the_handle_it_was_given(cx: &mut gpui::TestAppContext) {
        ...
        let first = cx.add_window(|window, cx| {
            ...
            crate::zzclaw_root(view, window, cx)
        });

...
pub struct ZzClawCommandIndex {
    pub section: usize,
    pub row: usize,
}

impl From<IndexPath> for ZzClawCommandIndex {
...
impl From<ZzClawCommandIndex> for IndexPath {
    fn from(index: ZzClawCommandIndex) -> Self {
        IndexPath::new(index.row).section(index.section)
    }
}

pub struct ZzClawCommandState {
    inner: Entity<CommandState>,
}

impl ZzClawCommandState {
    ...
    pub fn selected_index(&self, cx: &App) -> Option<ZzClawCommandIndex> {
    ...
    pub fn set_selected_index(
        &mut self,
        index: Option<ZzClawCommandIndex>,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
...
#[derive(Clone)]
pub struct ZzClawCommandItem {
    inner: CommandItem,
}

impl ZzClawCommandItem {
    pub fn new() -> Self {
        Self {
...
impl Default for ZzClawCommandItem {
    fn default() -> Self {
        Self::new()
    }
}

type ZzClawCommandSlot = dyn Fn(&mut Window, &mut App) -> AnyElement;
type ZzClawCommandQueryHandler = dyn Fn(&str, &mut Window, &mut App);
type ZzClawCommandIndexHandler = dyn Fn(ZzClawCommandIndex, &mut Window, &mut App);
type ZzClawCommandCancelHandler = dyn Fn(&mut Window, &mut App);

#[derive(IntoElement)]
pub struct ZzClawCommand {
    state: Entity<ZzClawCommandState>,
    items: Vec<ZzClawCommandItem>,
    searchable: bool,
    filterable: bool,
    placeholder: Option<SharedString>,
    max_h: Option<DefiniteLength>,
    bordered: bool,
    empty: Option<Rc<ZzClawCommandSlot>>,
    footer: Option<Rc<ZzClawCommandSlot>>,
    on_query: Option<Rc<ZzClawCommandQueryHandler>>,
    on_select: Option<Rc<ZzClawCommandIndexHandler>>,
    on_confirm: Option<Rc<ZzClawCommandIndexHandler>>,
    on_cancel: Option<Rc<ZzClawCommandCancelHandler>>,
    consume_cancel: bool,
}

impl ZzClawCommand {
    pub fn new(state: &Entity<ZzClawCommandState>) -> Self {
        Self {
            state: state.clone(),
            items: Vec::new(),
            searchable: true,
    ...
    pub fn item(mut self, item: ZzClawCommandItem) -> Self {
        self.items.push(item);
        self
    }

    pub fn items(mut self, items: impl IntoIterator<Item = ZzClawCommandItem>) -> Self {
    ...
    pub fn on_select(
        mut self,
        handler: impl Fn(ZzClawCommandIndex, &mut Window, &mut App) + 'static,
    ) -> Self {
...
impl RenderOnce for ZzClawCommand {
...
mod tests {
    ...
    use super::{ZzClawCommand, ZzClawCommandIndex, ZzClawCommandItem, ZzClawCommandState};

    struct CommandFixture {
        state: gpui::Entity<ZzClawCommandState>,
    }
    ...
    impl Render for CommandFixture {
        fn render(
            ...
            ZzClawCommand::new(&self.state).items([
                ZzClawCommandItem::new().label("SSH server"),
                ZzClawCommandItem::new().label("Local terminal"),
            ])
        }
    }
    ...
    struct CommandInteractionFixture {
        ...
        confirmed: Rc<RefCell<Vec<ZzClawCommandIndex>>>,
    }
    ...
    fn command_state_exposes_query_and_selection_without_component_types(cx: &mut TestAppContext) {
        ...
        let (fixture, cx) = cx.add_window_view(|window, cx| CommandFixture {
            state: cx.new(|cx| ZzClawCommandState::new(window, cx)),
        });
        ...
        cx.update(|window, cx| {
            ...
            assert_eq!(
                ...
                Some(ZzClawCommandIndex { section: 0, row: 0 })
            );
        ...
        assert_eq!(
            ...
            Some(ZzClawCommandIndex { section: 0, row: 1 })
        );
    ...
    fn escape_clears_then_cancels_without_leaking_and_enter_confirms(cx: &mut TestAppContext) {
        ...
        assert_eq!(
            ...
            &[ZzClawCommandIndex { section: 0, row: 1 }]
        );

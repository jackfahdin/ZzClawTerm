...
impl ZzClawDialog {
    ...
    pub fn confirm(mut self, footer: ZzClawDialogFooter) -> Self {
        self.inner = self
            .inner
    ...
    pub fn alert(mut self, action_label: impl Into<SharedString>) -> Self {
        ...
        self.inner = self
            ...
            .footer(zzclaw_dialog_action_footer(
                action_label,
...
pub struct ZzClawConfirmDialog {
    dialog: ZzClawDialog,
}

impl ZzClawConfirmDialog {
    pub fn new(dialog: ZzClawDialog, footer: ZzClawDialogFooter) -> Self {
        Self {
    ...
    pub fn into_dialog(self) -> ZzClawDialog {
        self.dialog
    }
}

pub struct ZzClawDialogFooter {
...
fn zzclaw_dialog_action_footer(
    action_label: SharedString,
    action_variant: ButtonVariant,
) -> DialogFooter {
    DialogFooter::new().child(zzclaw_dialog_action(action_label, action_variant))
}

fn zzclaw_dialog_action(action_label: SharedString, action_variant: ButtonVariant) -> DialogAction {
...
pub trait ZzClawDialogWindowExt {
    fn open_nya_dialog<F>(&mut self, cx: &mut App, build: F)
    where
        F: Fn(ZzClawDialog, &mut Window, &mut App) -> ZzClawDialog + 'static;
...
impl ZzClawDialogWindowExt for Window {
    fn open_nya_dialog<F>(&mut self, cx: &mut App, build: F)
    where
        F: Fn(ZzClawDialog, &mut Window, &mut App) -> ZzClawDialog + 'static,
    {
        self.open_dialog(cx, move |dialog, window, cx| {
            build(ZzClawDialog::from_component(dialog), window, cx).into_component()
        });
    }

    fn has_active_nya_dialog(&mut self, cx: &mut App) -> bool {
        // `gpui-component` reads its `Root` with an `expect`, so asking a window
        // that was not built with `zzclaw_root` panics rather than answering. A
        // window with no component root has no dialog, which is the answer every
        // caller wants.
        if self.root::<crate::root::ZzClawRoot>().flatten().is_none() {
            return false;
        }

...
///
/// This type is the stable ZzClawTerm boundary around gpui-component's editor;
/// desktop features never need to import gpui-component directly.
pub struct ZzClawDocumentEditorState {
        ...
        let subscription = cx.subscribe(&editor, |this, editor, event: &InputEvent, cx| {
            ...
            match event {
                InputEvent::Change => {
                    ...
                    cx.emit(ZzClawDocumentEditorEvent::Changed(value));
                }
                InputEvent::Blur => cx.emit(ZzClawDocumentEditorEvent::Blurred(value)),
...
impl EventEmitter<ZzClawDocumentEditorEvent> for ZzClawDocumentEditorState {}

impl Focusable for ZzClawDocumentEditorState {
    fn focus_handle(&self, cx: &App) -> FocusHandle {
        self.editor.read(cx).focus_handle(cx)
    }
}

impl Render for ZzClawDocumentEditorState {
...
#[derive(IntoElement)]
pub struct ZzClawDocumentEditor {
    state: Entity<ZzClawDocumentEditorState>,
}

impl ZzClawDocumentEditor {
    pub fn new(state: &Entity<ZzClawDocumentEditorState>) -> Self {
        Self {
            state: state.clone(),
        }
    }

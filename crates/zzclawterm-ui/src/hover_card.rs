...
use crate::{ZzClawPopoverAlign, ZzClawPopoverPlacement};

/// Hover-triggered rich content that stays open while the pointer moves from
/// the trigger into the card.
///
/// This is the stable ZzClawTerm boundary for `gpui-component`'s HoverCard. It is
/// intended for concise, pointer-interactive previews; click-driven workflows
/// should continue to use [`crate::ZzClawPopover`].
#[derive(IntoElement)]
pub struct ZzClawHoverCard {
    id: SharedString,
    trigger: AnyElement,
    content: AnyElement,
    anchor: Anchor,
...
#[derive(Default)]
struct ZzClawHoverCardAnchorState {
...
impl Render for ZzClawHoverCardState {
    fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
        div()
    }
}

impl RenderOnce for ZzClawHoverCard {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let state = window.use_keyed_state(id.clone(), cx, |_, _| {
            ZzClawHoverCardState::new(self.open_delay, self.close_delay)
        });
...
mod tests {
    ...
    fn default_open_delay_avoids_triggering_on_a_brief_pass() {
        let card = ZzClawHoverCard::new("hover-card-delay", div(), div());
    ...
    fn side_placement_can_be_configured_independently_from_corner_anchoring() {
        let card = ZzClawHoverCard::new("hover-card-side", div(), div())
            .placement(ZzClawPopoverPlacement::Right)
            .align(ZzClawPopoverAlign::Center)
            .offset(px(6.));
        assert_eq!(card.placement, Some(ZzClawPopoverPlacement::Right));
        assert_eq!(card.align, ZzClawPopoverAlign::Center);
        assert_eq!(card.offset, px(6.));
    }

    #[test]
    fn placement_is_unset_by_default_so_existing_callers_stay_corner_anchored() {
        let card = ZzClawHoverCard::new("hover-card-default", div(), div());

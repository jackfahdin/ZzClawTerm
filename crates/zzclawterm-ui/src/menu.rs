...
pub(crate) fn zzclaw_popup_menu_appearance() -> PopupMenuAppearance {
...
#[derive(Clone)]
pub struct ZzClawMenuItem {
    kind: ZzClawMenuItemKind,
    label: SharedString,
...
impl ZzClawMenuItem {
    pub fn action(label: impl Into<SharedString>) -> Self {
        Self {
            kind: ZzClawMenuItemKind::Action,
            label: label.into(),
            icon_path: None,
            icon_color: None,
            shortcut: None,
    ...
    pub fn separator() -> Self {
        Self {
            kind: ZzClawMenuItemKind::Separator,
            ..Self::action("")
        }
    }
    ...
    pub fn submenu(label: impl Into<SharedString>, items: Vec<Self>) -> Self {
        Self {
            kind: ZzClawMenuItemKind::Submenu(items),
            ..Self::action(label)
        }
    }
    ...
    #[doc(hidden)]
    pub fn children(&self) -> Option<&[ZzClawMenuItem]> {
        match &self.kind {
            ZzClawMenuItemKind::Submenu(items) => Some(items.as_slice()),
            _ => None,
        }
    }
...
type DropdownItemsBuilder = Rc<dyn Fn(&mut Window, &mut App) -> Vec<ZzClawMenuItem>>;

impl ZzClawDropdownMenu {
    pub fn new(id: impl Into<SharedString>) -> Self {
        Self {
            ...
            selected: false,
            disabled: false,
            anchor: ZzClawMenuAnchor::default(),
            min_width: None,
            max_width: None,
            max_height: None,
            scrollable: None,
    ...
    pub fn anchor(mut self, anchor: ZzClawMenuAnchor) -> Self {
        self.anchor = anchor;
        self
    }
    ...
    pub fn items_dynamic(
        mut self,
        builder: impl Fn(&mut Window, &mut App) -> Vec<ZzClawMenuItem> + 'static,
    ) -> Self {
        self.items = Rc::new(builder);
        self
    }

    pub fn items(mut self, items: impl IntoIterator<Item = ZzClawMenuItem>) -> Self {
...
impl RenderOnce for ZzClawDropdownMenu {
...
/// [`ZzClawContextMenu::new_dynamic`]: reset the target from a capture-phase
...
#[derive(IntoElement)]
pub struct ZzClawContextMenu<E>
where
...
impl<E> ZzClawContextMenu<E>
where
    E: InteractiveElement + ParentElement + Styled + IntoElement + 'static,
{
    pub fn new(element: E, items: impl IntoIterator<Item = ZzClawMenuItem>) -> Self {
        let items: Vec<_> = items.into_iter().collect();
        Self {
            element,
    ...
    pub fn new_dynamic(
        element: E,
        items_builder: impl Fn(&mut Window, &mut App) -> Vec<ZzClawMenuItem> + 'static,
    ) -> Self {
        Self {
            element,
...
fn popup_menu_should_scroll(items: &[ZzClawMenuItem], override_value: Option<bool>) -> bool {
...
impl<E> RenderOnce for ZzClawContextMenu<E>
where
    E: InteractiveElement + ParentElement + Styled + IntoElement + 'static,
{
    fn render(self, _window: &mut Window, _cx: &mut App) -> impl IntoElement {
...
mod tests {
    ...
    use super::{
        ZzClawContextMenu, ZzClawMenuItem, ZzClawMenuItemKind, popup_menu_should_scroll,
            ...
            ZzClawContextMenu::new_dynamic(
                ...
                move |_, _| match target_for_menu.get() {
                    1 => {
                        ...
                        vec![ZzClawMenuItem::action("Parent").on_click(move |_, _, _| {
                            parent_triggered.set(true);
                        })]
                    }
                    2 => {
                        let entry_triggered = entry_triggered.clone();
                        vec![ZzClawMenuItem::action("Entry").on_click(move |_, _, _| {
                            entry_triggered.set(true);
                        })]
                    }
                    _ => {
                        let current_triggered = current_triggered.clone();
                        vec![ZzClawMenuItem::action("Current").on_click(move |_, _, _| {
    ...
    fn menu_item_builders_preserve_behavior_flags() {
        let item = ZzClawMenuItem::action("Delete")
        ...
        assert!(matches!(item.kind, ZzClawMenuItemKind::Action));
    ...
    fn popup_menus_scroll_only_for_long_flat_item_lists() {
        ...
        long_with_submenu.push(ZzClawMenuItem::submenu(
            "Move",
            vec![ZzClawMenuItem::action("Group")],
        ));
    ...
    fn context_menu_retains_optional_minimum_width() {
        let menu =
            ZzClawContextMenu::new(div(), [ZzClawMenuItem::action("Open")]).min_width(px(200.));
    ...
    fn context_menu_supports_the_sftp_browser_minimum_width() {
        let menu =
            ZzClawContextMenu::new(div(), [ZzClawMenuItem::action("Preview")]).min_width(px(208.));
    ...
    fn context_submenu_reserves_parent_and_child_width_near_right_edge() {
        let items = vec![ZzClawMenuItem::submenu(
            "Move to category",
            vec![ZzClawMenuItem::action("Category")],
        )];
    ...
    fn context_submenu_uses_configured_parent_and_child_widths() {
        let items = vec![
            ZzClawMenuItem::submenu(
                "Move",
                vec![ZzClawMenuItem::action("Folder").label_indent(px(10.))],
            )
    ...
    fn submenu_retains_nested_items() {
        let item = ZzClawMenuItem::submenu(
            "Move",
            vec![
                ZzClawMenuItem::action("Group A"),
                ZzClawMenuItem::separator(),
            ],
        );

        let ZzClawMenuItemKind::Submenu(items) = item.kind else {

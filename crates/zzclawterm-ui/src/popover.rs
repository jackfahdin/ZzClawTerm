...
type ZzClawPopoverOpenHandler = Rc<dyn Fn(&bool, &mut Window, &mut App)>;
type ZzClawPopoverOutsideHandler = Rc<dyn Fn(&mut Window, &mut App)>;

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ZzClawPopoverPlacement {
    Top,
    Bottom,
    Left,
    Right,
...
impl From<ZzClawPopoverPlacement> for gpui_base::Placement {
    fn from(value: ZzClawPopoverPlacement) -> Self {
        match value {
            ZzClawPopoverPlacement::Top => Self::Top,
            ZzClawPopoverPlacement::Bottom => Self::Bottom,
            ZzClawPopoverPlacement::Left => Self::Left,
            ZzClawPopoverPlacement::Right => Self::Right,
        }
    }
}

#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub enum ZzClawPopoverAlign {
    Start,
    #[default]
    Center,
    End,
...
impl From<ZzClawPopoverAlign> for gpui_base::Align {
    fn from(value: ZzClawPopoverAlign) -> Self {
        match value {
            ZzClawPopoverAlign::Start => Self::Start,
            ZzClawPopoverAlign::Center => Self::Center,
            ZzClawPopoverAlign::End => Self::End,
        }
    }
}

#[derive(Default)]
struct ZzClawPopoverAnchorState {
    bounds: Bounds<Pixels>,
    captured: bool,
}

#[derive(IntoElement)]
struct ZzClawPopoverTrigger {
    element: AnyElement,
    selected: bool,
}

impl Selectable for ZzClawPopoverTrigger {
...
impl RenderOnce for ZzClawPopoverTrigger {
...
#[derive(IntoElement)]
pub struct ZzClawPopover {
    id: SharedString,
    trigger: AnyElement,
    content: AnyElement,
    anchor: Anchor,
    placement: Option<ZzClawPopoverPlacement>,
    align: ZzClawPopoverAlign,
    offset: Pixels,
    open: Option<bool>,
    appearance: bool,
    overlay_closable: bool,
    trigger_click: bool,
    on_open_change: Option<ZzClawPopoverOpenHandler>,
    on_click_outside: Option<ZzClawPopoverOutsideHandler>,
}
...
impl ZzClawPopover {
    pub fn new(
        ...
        Self {
            ...
            placement: None,
            align: ZzClawPopoverAlign::Center,
            offset: px(0.),
            open: None,
            appearance: true,
    ...
    pub fn align(mut self, align: ZzClawPopoverAlign) -> Self {
        self.align = align;
        self
    }
...
impl RenderOnce for ZzClawPopover {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let anchor_state = window.use_keyed_state((id.clone(), "side-anchor"), cx, |_, _| {
            ZzClawPopoverAnchorState::default()
        });
...
impl ZzClawPopover {
    fn render_corner(self) -> impl IntoElement {
        let trigger = ZzClawPopoverTrigger {
...
mod tests {
    ...
    use crate::ZzClawScrollArea;

    use super::{ZzClawPopover, ZzClawPopoverAlign, ZzClawPopoverPlacement};
                ...
                .child(
                    ZzClawPopover::new(
                        ...
                        div()
                            ...
                            .overflow_hidden()
                            .child(
                                ZzClawScrollArea::new("side-popover-scroll")
                                    .max_h(px(96.))
                    ...
                    .placement(ZzClawPopoverPlacement::Left)
        ...
        fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl gpui::IntoElement {
            div()
                ...
                .child(
                    ...
                    .placement(ZzClawPopoverPlacement::Right)
                    .align(ZzClawPopoverAlign::Start)
                    .offset(px(4.))
                    .appearance(false)
                    .overlay_closable(false)
    ...
    fn anchor_defaults_to_top_left_and_can_be_overridden() {
        let default = ZzClawPopover::new("default", div(), div());
        assert_eq!(default.anchor, Anchor::TopLeft);

        let anchored = ZzClawPopover::new("anchored", div(), div()).anchor(Anchor::TopRight);
    ...
    fn side_placement_can_be_configured_independently_from_corner_anchoring() {
        let popover = ZzClawPopover::new("side", div(), div())
            .placement(ZzClawPopoverPlacement::Left)
            .align(ZzClawPopoverAlign::Start)
            .offset(px(4.));
        assert_eq!(popover.placement, Some(ZzClawPopoverPlacement::Left));
        assert_eq!(popover.align, ZzClawPopoverAlign::Start);

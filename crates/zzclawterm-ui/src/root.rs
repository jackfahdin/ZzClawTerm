...
///
/// Keep this alias inside `zzclawterm-ui` so feature modules do not depend on the
...
/// view layer.
pub type ZzClawRoot = gpui_kit::component::Root;

pub type ZzClawWindowHandle = WindowHandle<ZzClawRoot>;

struct ZzClawRootContent {
    view: AnyView,
}

impl ZzClawRootContent {
...
mod tests {
    ...
    use crate::{
        ZzClawDialogFooter, ZzClawDialogWindowExt as _, ZzClawInputEvent, ZzClawInputShell,
        ZzClawInputState, ZzClawNumberInput, ZzClawNumberInputOptions, ZzClawNumberInputState,
        ZzClawSearchInput, zzclaw_root,
    };
    ...
    struct InputFocusFixture {
        first: gpui::Entity<ZzClawInputState>,
        second: gpui::Entity<ZzClawInputState>,
        ordinary: gpui::Entity<ZzClawInputState>,
        masked: gpui::Entity<ZzClawInputState>,
        number: gpui::Entity<ZzClawNumberInputState>,
    ...
    impl InputFocusFixture {
        fn new(cx: &mut Context<Self>) -> Self {
            let first = cx.new(|cx| ZzClawInputState::new(cx, "").placeholder("First"));
            let second = cx.new(|cx| ZzClawInputState::new(cx, "").placeholder("第二个搜索框"));
            let ordinary = cx.new(|cx| ZzClawInputState::new(cx, "Default value"));
            let masked = cx.new(|cx| {
                ZzClawInputState::new(cx, "secret")
                    .placeholder("密码")
                    .masked(true)
            });
            let number = cx.new(|cx| {
                ZzClawNumberInputState::new(cx, "1", ZzClawNumberInputOptions::default())
            });
            Self {
                first,
                second,
        ...
        fn subscribe_to_input_events(&mut self, cx: &mut Context<Self>) {
            self._subscriptions = vec![
                cx.subscribe(&self.first, |this, _, event: &ZzClawInputEvent, _| {
                    if matches!(event, ZzClawInputEvent::Blurred(_)) {
                        this.first_blurs += 1;
                    }
                }),
                cx.subscribe(&self.second, |this, _, event: &ZzClawInputEvent, _| {
                    if matches!(event, ZzClawInputEvent::Blurred(_)) {
                        this.second_blurs += 1;
                    }
                }),
    ...
    impl Render for InputFocusFixture {
        fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
            ...
            div()
                ...
                .child(
                    div().w(px(220.)).child(
                        ZzClawSearchInput::new("focus-first", &self.first).trailing(
                            div()
                ...
                .child(
                    div()
                        .w(px(220.))
                        .child(ZzClawSearchInput::new("focus-second", &self.second)),
                )
                .child(
                    div()
                        .w(px(220.))
                        .child(ZzClawInputShell::new("focus-ordinary", &self.ordinary)),
                )
                .child(
                    div()
                        .w(px(220.))
                        .child(ZzClawInputShell::new("focus-masked", &self.masked)),
                )
                .child(
                    div()
                        .w(px(160.))
                        ...
                        .child(ZzClawNumberInput::new(&self.number)),
                )
                .child(
                    div()
                        .w(px(220.))
    ...
    #[gpui::test]
    fn zzclaw_root_renders_component_dialog_layer(cx: &mut TestAppContext) {
        cx.update(gpui_kit::init);

        let (_, cx) = cx.add_window_view(|window, cx| {
            let view = cx.new(|_| RootContentFixture);
            zzclaw_root(view, window, cx)
        });
    ...
    fn dialog_layer_inherits_the_application_font_fallbacks_and_size(cx: &mut TestAppContext) {
        ...
        let view = cx.new(|_| RootContentFixture);
        let (_, cx) = cx.add_window_view(move |window, cx| zzclaw_root(view, window, cx));
        cx.update(|window, cx| {
    ...
    #[gpui::test]
    fn zzclaw_dialog_blocks_lower_pointer_events_while_open_and_preserves_clicks(
    ...
    #[gpui::test]
    fn zzclaw_confirm_dialog_renders_footer_actions(cx: &mut TestAppContext) {
        cx.update(gpui_kit::init);
        ...
        cx.update(|window, cx| {
            window.open_nya_dialog(cx, |dialog, _, _| {
                dialog
                    .title("Delete Item")
                    .confirm(ZzClawDialogFooter::new("Cancel", "Delete"))
                    .content(
                        div()
    ...
    #[gpui::test]
    fn zzclaw_danger_confirm_dialog_renders_footer_actions(cx: &mut TestAppContext) {
        cx.update(gpui_kit::init);
        ...
        cx.update(|window, cx| {
            window.open_nya_dialog(cx, |dialog, _, _| {
                dialog
                    .title("Delete Folder")
                    .confirm(ZzClawDialogFooter::new("Cancel", "Delete").danger())
                    .content(
                        div()
    ...
    #[gpui::test]
    fn zzclaw_alert_dialog_renders_action_footer(cx: &mut TestAppContext) {
        cx.update(gpui_kit::init);
    ...
    #[gpui::test]
    fn zzclaw_dialog_close_button_dismisses_the_dialog(cx: &mut TestAppContext) {
        cx.update(gpui_kit::init);
    ...
    ///
    /// This is the shape a ZzClawTerm list has to compose: the list carries one
    /// `ZzClawContextMenu`, resets the target on capture, and each row re-aims it
    ...
    impl Render for ListContextMenuFixture {
        fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
            ...
            crate::ZzClawContextMenu::new_dynamic(
                div()
                ...
                move |_, _| {
                    vec![
                        crate::ZzClawMenuItem::action(*build_target.borrow()).on_click(
                            |_, window, cx| {
                                window.open_nya_dialog(cx, |dialog, _, _| {
                                    dialog
                                        .width(400.)
                                        ...
                                        .confirm(
                                            ZzClawDialogFooter::new("Cancel", "Delete").danger(),
                                        )
                                        .content(
                                            div()
                                                .debug_selector(|| {
                                                    "focus-dialog-content".to_string()
                                                })
                                                .h(px(60.))
                                                .child("Delete this?"),
                                        )

...
//! application. Communication is the typed IPC protocol in `zzclawterm-remote-desktop`:
...
fn main() {
    match std::env::var("ZZCLAWTERM_VNC_HELPER_TEST_MODE").as_deref() {
...
fn spawn_stdout_writer(
    ...
    thread::Builder::new()
        .name("zzclawterm-vnc-stdout".to_string())
        .spawn(move || {
...
fn spawn_worker(
    ...
    thread::Builder::new()
        .name(format!("zzclawterm-vnc-{session_id}"))
        .spawn(move || {

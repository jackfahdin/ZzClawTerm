...
ZzClawTerm 的原生界面位于 `crates/zzclawterm-desktop`，共享控件位于 `crates/zzclawterm-ui`。这里的“桌面层”同时包含 GPUI 状态、视图、窗口交互、平台适配和后台任务结果的接入。

## 入口与窗口

`zzclawterm-app` 创建根窗口和 `AppShell`。存储启动完成后，`AppShell` 创建 `ZzClawTermApp` Entity 并启动工作区恢复。
...
`ZzClawTermApp` 组合各个 focused feature state。只操作一个领域的方法优先放到对应 state 上；需要通知 GPUI、访问窗口或协调多个领域时，再由 `ZzClawTermApp` 提供薄适配。

遵守以下所有权规则：

- 每份可变状态只有一个权威 owner。
- 不在 `ZzClawTermApp`/feature state 和 Entity store 中维护双写镜像。
- 由视图直接读取当前状态，不在 render 中发布再读回快照。
- 跨线程结果通过 typed event 或 typed task result 进入 GPUI update。

## 视图与控件

构造 GPUI element 的 helper 留在 view 或桌面 feature 中，不要为减少 `impl ZzClawTermApp` 数量而把视图构造移动到纯状态模型。

普通输入、选择、菜单、开关和对话框使用 `zzclawterm-ui` 暴露的稳定组件 API。桌面 feature 不直接依赖 `gpui-component`。普通文本字段使用 `ZzClawInput`/`ZzClawInputState` 或 `features/text_inputs.rs` 的 id registry，并为输入框提供明确尺寸。
...
- `zzclawterm-terminal` 维护终端网格、scrollback、控制序列、搜索和图形协议状态。
- `zzclawterm-terminal-gpui` 负责像素布局、键盘事件转换、选区、高亮、图片和绘制。

...
普通输入、选择、菜单、开关和对话框使用 `zzclawterm-ui` 暴露的稳定组件 API。桌面 feature 不直接依赖 `gpui-kit`。普通文本字段使用 `ZzClawInput`/`ZzClawInputState` 或 `features/text_inputs.rs` 的 id registry，并为输入框提供明确尺寸。

...
ZzClawTerm 内置 AI 助手面板，可以从终端上下文、选中文本、文件操作或手动输入发起请求。它支持两种工作模式：**Ask** 和 **Agent**。
...
拆开之后终端保留真实的执行记录，最终回答负责解释结果和下一步建议，多步排障和部署检查更容易复盘。
...
一条命令的风险等级不是模型单方面说了算。ZzClawTerm 会取**模型自报等级**和**本地规则判定**中较高的一个作为生效等级，所以模型低估风险时本地规则仍会拦住它。命令卡片上的风险说明会同时列出两个来源。
...
生效等级决定命令能不能直接跑，取决于 **设置 → AI → Agent Settings** 里的 **命令执行策略**：

| 策略 | 行为 |
|------|------|
| 执行前确认 | 每条命令执行前都需要你确认 |
| 全自动 | 不再确认，直接执行 |
| 智能放行 | 生效等级不高于 **智能模式自动执行上限** 时自动执行，否则转为等待审批 |

**智能放行** 下还有一条硬规则：**极高风险始终需要人工确认**，即使你把自动执行上限调到了最高。
...
当命令被判为高风险时，AI 可能会同时提供更安全的替代命令供选择。

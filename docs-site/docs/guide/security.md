...
ZzClawTerm 会在本地保存连接相关配置，并对敏感内容做加密处理后再写入本地存储。常见的敏感数据包括：
...
主密码是 ZzClawTerm 桌面端最重要的一项本地保护能力。
...
- 锁屏界面会要求输入这个密码才能解锁
- **云同步** 相关操作才允许被启用
- ZzClawTerm 的加密 `.zz` 配置导入 / 导出也会以它作为前提条件
...
它的定位是跨设备同步常用配置，配合本地 `.zz` 导出完成备份和迁移，不是多人协作或字段级合并工具。
...
应用锁定状态会在主工作区、模态子窗口和空闲检测之间保持一致。进入锁定后，终端内容和敏感面板会被遮挡；解锁前不要把它当作远端系统的权限控制，它保护的是本机 ZzClawTerm 工作区视图和本地敏感操作入口。

在共享办公环境或演示现场使用时，把主密码、屏幕锁定和空闲自动锁定一起开启。

## SSH 主机密钥策略

SSH 首次连接未知主机时，ZzClawTerm 支持三种策略：
...
安装模式下主要用户数据保存在 `~/.zzclawterm/zzclawterm.redb`；portable 模式使用程序旁 `data/config/` 下的配置数据。
...
**导出配置** 把全部配置压缩并加密为 `.zz`，用于迁移或离线留存，导入导出都依赖主密码。**导出诊断包** 打包近期日志和运行时摘要用于排查问题；诊断包在分享前先检查内容。两个入口和日志级别、日志保留时间、打开日志目录都在 **设置 → 常规**。
...
- 对 AI provider 凭据、同步 provider 凭据和 `.zz` 备份文件，按和 SSH 凭据相同的敏感级别管理

...
ZzClawTerm 不只是 SSH 客户端，而是一个把多类终端与远程桌面工作流放到同一工作区中的桌面应用。当前支持六类会话：
...
本地终端把本机 shell 工作流放进 ZzClawTerm 工作区，例如：
...
它保留的是 ZzClawTerm 的终端工作区能力，但不会附带 SSH 特有的安全和文件能力。因此通常不会有：
...
串口会话同样可以放进标签页与分屏工作区，一边观察串口输出，一边在另一个 SSH 或本地终端里执行命令。
...
ZzClawTerm 的优势之一，是允许你把这些类型混合放进同一工作区，例如：
...
工作区是统一的，但能力边界取决于底层会话类型，所以某些面板和菜单项只在特定类型的标签页里出现。

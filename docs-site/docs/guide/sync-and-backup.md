...
上传到云端的是**加密后的可移植快照**，加密依赖主密码派生的密钥。
...
把当前设备的可移植配置快照上传到云端。用于刚完成一组设置调整后，或需要明确控制何时写入云端时。

### 立即拉取

把云端当前最新同步快照拉回本地并应用。用于另一台设备已经推送过更新，需要让当前设备显式对齐时。
...
把远端快照拉回当前设备并应用。**当前设备上未同步的本地变更会丢失**，选它之前先确认云端版本是要保留的那份。

### 上传本地版本

强制用当前设备的本地快照覆盖远端最新状态。**远端那份变更会被覆盖**，其他设备后续会以这份状态为准。
...
所以它同步的是可移植配置，让多台设备共享当前核心配置状态，不是完整的桌面会话镜像。

...
Ordinary inputs, selects, menus, switches, and dialogs use the stable component API exposed by `zzclawterm-ui`. Desktop features do not depend directly on `gpui-component`. Ordinary text fields use `ZzClawInput`/`ZzClawInputState` or the id registry in `features/text_inputs.rs`, with definite dimensions around the input.

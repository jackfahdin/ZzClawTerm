...
- 手动双击 ZzXsrv.exe 的默认模式：裸 rootful 无窗口管理器（X 应用无标题栏、不可拖动，按记忆几何位置摆放可跑出屏幕外，实机 2026-08-25 用户验收时实测踩到）。产品化时应默认 multiwindow（安装包快捷方式加 `-multiwindow`，或评估 exe 缺省行为）——**已由 M5b（2026-08-25，zz3）解决：开始菜单快捷方式默认 `-multiwindow`；exe 本体无参缺省行为的评估仍留后续版本**

...
1. M6a：netem 25/50/100ms 三档比值 ≥0.95 且回环无 5% 回归；ZzSshCore 全部测试绿
2. M6b：设置项 + 接线 + 验收清单文档交付；主仓 45 基线 + 新增全绿
3. 任务 0 调研结论入库（docs/superpowers/specs/ 附录或 ZzSshCore docs）
4. 规格条目核销；代码审查通过
5. ⏸ 人工验收（V0.2 清单 + X11 M5 清单）由用户执行
